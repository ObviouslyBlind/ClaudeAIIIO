...
Catalog: `game/SKILLS.md`. Grab list: `two-harbors-job-index`.
...
| `threejs-game` / `threejs-game-studio` new Vite app | Never. `create-threejs-game` was stripped |
| R3F / React Three Fiber | Harbour client is vanilla JS |
| Procedural planet / black hole skills | Shelf only. Two authored islands |
| FFT ocean on phone Low | Cheap Gerstner / `water-optics` notes; no default post-process |
...
Which file to open: **`two-harbors-job-index`**. Then this table.
...
| Water / sea | `threejs-water-optics`, `threejs-spectral-ocean` after this file |
| Click / raycast | `threejs-interaction` after `two-harbors-controls` |
| Frame time / instancing | `three-best-practices`, `threejs-perf` |
