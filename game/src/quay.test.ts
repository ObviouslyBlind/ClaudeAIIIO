...
describe("quay harbour dressing", () => {
  ...
  it("keeps spawn-readable sage dinghy hulls with kraft gunwales in the basin", () => {
    ...
    for (const boat of boats) {
      ...
      expect(hg.parameters.width).toBeGreaterThanOrEqual(1.4);
      expect(hg.parameters.width).toBeLessThan(2.4);
      expect(hg.parameters.height).toBeGreaterThanOrEqual(0.4);
      expect(hg.parameters.height).toBeLessThan(0.9);
      expect(hg.parameters.depth).toBeGreaterThanOrEqual(3.5);
      expect(hg.parameters.depth).toBeLessThan(5);
  ...
  it("keeps unread giant basin extras off the live north quay", () => {
    const spec = ISLANDS.north;
    const added: THREE.Object3D[] = [];
    const scene = { add(obj: THREE.Object3D) { added.push(obj); } };
    const root = makeQuay(spec, { scene, heightAt });
    const unread = ["brow", "plate", "ring", "funnel", "cargo", "teal", "clay", "green"];
    for (const kind of unread) {
      expect(root.children.find((c) => c.userData?.dress === kind)).toBeUndefined();
    }
    expect(root.children.find((c) => c.userData?.dress === "buoy")).toBeTruthy();
    expect(root.children.filter((c) => c.userData?.kind === "dinghy").length).toBe(2);
  });
  ...
  it("stands a spawn-readable rust buoy in the north basin channel with the dinghies", () => {
    ...
    expect(bg.parameters.width).toBeGreaterThanOrEqual(0.7);
    expect(bg.parameters.width).toBeLessThan(1.4);
    expect(bg.parameters.height).toBeGreaterThanOrEqual(0.8);
    expect(bg.parameters.height).toBeLessThan(1.5);
    const bm = (body as THREE.Mesh).material as THREE.MeshLambertMaterial;
    expect(bm.color.getHex()).toBe(0x6e2e22);
    expect(buoy!.position.y).toBeGreaterThan(0.3);
    expect(buoy!.position.y).toBeLessThan(1.2);
