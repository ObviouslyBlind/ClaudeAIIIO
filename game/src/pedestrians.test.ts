...
describe("harbour PAPER pedestrians", () => {
  ...
  it("carries a tiny kraft PAPER sack on quay walkers; tin, pin, basket, loaf, parcel, flask, bundle, can, match, pipe, cigar, gourd, torch stay", () => {
    const kraftSack = new Set([0xc4b496, 0xc4a574, 0x4a3220]);
    const figure = makePaperPerson(0);
    expect(partsOf(figure).get("sack")).toBeUndefined();
    expect(partsOf(figure).get("tin")).toBeUndefined();
    expect(partsOf(figure).get("pin")).toBeUndefined();
    expect(partsOf(figure).get("basket")).toBeUndefined();
    expect(partsOf(figure).get("loaf")).toBeUndefined();
    ...
    for (const p of quay) {
      const parts = partsOf(p.mesh);
      expect(parts.get("sack")).toBeGreaterThanOrEqual(1);
      expect(parts.get("tin")).toBeGreaterThanOrEqual(1);
      expect(parts.get("pin")).toBeGreaterThanOrEqual(1);
      expect(parts.get("basket")).toBeGreaterThanOrEqual(1);
      expect(parts.get("loaf")).toBeGreaterThanOrEqual(1);
      ...
      expect(parts.get("boot")).toBe(2);
      p.mesh.traverse((obj) => {
        if (obj.userData?.part !== "sack") return;
        const mesh = obj as THREE.Mesh;
        expect(mesh.userData.mode).toBe("PAPER");
        expect(mesh.geometry.type).toBe("BoxGeometry");
        expect((mesh.material as THREE.MeshLambertMaterial).type).toBe("MeshLambertMaterial");
        const hex = (mesh.material as THREE.MeshLambertMaterial).color.getHex();
        expect(isGrey(hex)).toBe(false);
        expect(kraftSack.has(hex)).toBe(true);
        const { width, height, depth } = (mesh.geometry as THREE.BoxGeometry).parameters;
        expect(width).toBeLessThan(0.18);
        expect(height).toBeLessThan(0.18);
        expect(depth).toBeLessThan(0.18);
    ...
    for (const p of verge) {
      expect(partsOf(p.mesh).get("sack")).toBeUndefined();
    }
  });

  it("carries a tiny kraft PAPER pouch on quay walkers; tin, pin, basket, loaf, parcel, flask, bundle, can, match, pipe, cigar, gourd, torch, sack stay", () => {
    const kraftPouch = new Set([0xc4b496, 0xc4a574, 0x4a3220]);
    const figure = makePaperPerson(0);
    expect(partsOf(figure).get("pouch")).toBeUndefined();
    expect(partsOf(figure).get("tin")).toBeUndefined();
    expect(partsOf(figure).get("pin")).toBeUndefined();
    expect(partsOf(figure).get("basket")).toBeUndefined();
    expect(partsOf(figure).get("loaf")).toBeUndefined();
    ...
    for (const p of quay) {
      const parts = partsOf(p.mesh);
      expect(parts.get("pouch")).toBeGreaterThanOrEqual(1);
      expect(parts.get("tin")).toBeGreaterThanOrEqual(1);
      expect(parts.get("pin")).toBeGreaterThanOrEqual(1);
      expect(parts.get("basket")).toBeGreaterThanOrEqual(1);
      expect(parts.get("loaf")).toBeGreaterThanOrEqual(1);
      ...
      expect(parts.get("boot")).toBe(2);
      p.mesh.traverse((obj) => {
        if (obj.userData?.part !== "pouch") return;
        const mesh = obj as THREE.Mesh;
        expect(mesh.userData.mode).toBe("PAPER");
        expect(mesh.geometry.type).toBe("BoxGeometry");
        expect((mesh.material as THREE.MeshLambertMaterial).type).toBe("MeshLambertMaterial");
        const hex = (mesh.material as THREE.MeshLambertMaterial).color.getHex();
        expect(isGrey(hex)).toBe(false);
        expect(kraftPouch.has(hex)).toBe(true);
        const { width, height, depth } = (mesh.geometry as THREE.BoxGeometry).parameters;
        expect(width).toBeLessThan(0.18);
        expect(height).toBeLessThan(0.18);
        expect(depth).toBeLessThan(0.18);
        ...
        expect(mesh.position.y).toBeLessThan(1.25);
        expect(Math.abs(mesh.position.x)).toBeGreaterThan(0.28);
        expect(mesh.position.distanceTo(new THREE.Vector3(-0.45, 1.10, 0.55))).toBeGreaterThan(0.12);
        expect(mesh.position.distanceTo(new THREE.Vector3(0.45, 1.10, -0.32))).toBeGreaterThan(0.12);
        expect(mesh.position.distanceTo(new THREE.Vector3(-0.45, 1.10, -0.32))).toBeGreaterThan(0.12);
        expect(mesh.position.distanceTo(new THREE.Vector3(0.45, 1.10, 0.32))).toBeGreaterThan(0.12);
        expect(mesh.position.distanceTo(new THREE.Vector3(-0.38, 0.54, -0.40))).toBeGreaterThan(0.12);
    ...
    for (const p of verge) {
      expect(partsOf(p.mesh).get("pouch")).toBeUndefined();
    }
  });

  it("carries a tiny kraft PAPER cane on quay walkers; tin, pin, basket, loaf, parcel, flask, bundle, can, match, pipe, cigar, gourd, torch, sack, pouch stay", () => {
    const kraftCane = new Set([0xc4b496, 0xc4a574, 0x4a3220]);
    const figure = makePaperPerson(0);
    expect(partsOf(figure).get("cane")).toBeUndefined();
    expect(partsOf(figure).get("tin")).toBeUndefined();
    expect(partsOf(figure).get("pin")).toBeUndefined();
    expect(partsOf(figure).get("basket")).toBeUndefined();
    expect(partsOf(figure).get("loaf")).toBeUndefined();
    ...
    for (const p of quay) {
      const parts = partsOf(p.mesh);
      expect(parts.get("cane")).toBeGreaterThanOrEqual(1);
      expect(parts.get("tin")).toBeGreaterThanOrEqual(1);
      expect(parts.get("pin")).toBeGreaterThanOrEqual(1);
      expect(parts.get("basket")).toBeGreaterThanOrEqual(1);
      expect(parts.get("loaf")).toBeGreaterThanOrEqual(1);
      ...
      expect(parts.get("boot")).toBe(2);
      p.mesh.traverse((obj) => {
        if (obj.userData?.part !== "cane") return;
        const mesh = obj as THREE.Mesh;
        expect(mesh.userData.mode).toBe("PAPER");
        expect(mesh.geometry.type).toBe("BoxGeometry");
        expect((mesh.material as THREE.MeshLambertMaterial).type).toBe("MeshLambertMaterial");
        const hex = (mesh.material as THREE.MeshLambertMaterial).color.getHex();
        expect(isGrey(hex)).toBe(false);
        expect(kraftCane.has(hex)).toBe(true);
        const { width, height, depth } = (mesh.geometry as THREE.BoxGeometry).parameters;
        expect(width).toBeLessThan(0.18);
        expect(height).toBeLessThan(0.18);
        expect(depth).toBeLessThan(0.18);
    ...
    for (const p of verge) {
      expect(partsOf(p.mesh).get("cane")).toBeUndefined();
    }
  });
