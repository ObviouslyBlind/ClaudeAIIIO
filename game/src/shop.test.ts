...
function shopLedgers(root: THREE.Object3D) {
  const out: THREE.Object3D[] = [];
  root.traverse((obj) => {
    if (obj.userData?.part === "ledger" && obj.name === "shop-ledger") {
      out.push(obj);
    }
  });
  return out;
...
function shopInkwells(root: THREE.Object3D) {
  const out: THREE.Object3D[] = [];
  root.traverse((obj) => {
    if (obj.userData?.part === "inkwell" && obj.name === "shop-inkwell") {
      out.push(obj);
    }
  });
  return out;
...
function shopRulers(root: THREE.Object3D) {
  const out: THREE.Object3D[] = [];
  root.traverse((obj) => {
    if (obj.userData?.part === "ruler" && obj.name === "shop-ruler") {
      out.push(obj);
    }
  });
  return out;
...
function shopEnvelopes(root: THREE.Object3D) {
  const out: THREE.Object3D[] = [];
  root.traverse((obj) => {
    if (obj.userData?.part === "envelope" && obj.name === "shop-envelope") {
      out.push(obj);
    }
  });
  return out;
...
function shopWafers(root: THREE.Object3D) {
  const out: THREE.Object3D[] = [];
  root.traverse((obj) => {
    if (obj.userData?.part === "wafer" && obj.name === "shop-wafer") {
      out.push(obj);
    }
  });
  return out;
...
describe("shop PAPER kraft ledger", () => {
  it("puts one tiny kraft PAPER ledger on the shop counter, soap and dustpan remain", () => {
    const scene = new THREE.Scene();
    const interior = makeInteriorScene();
    scene.add(interior);
    dressShop(scene);
    ...
    const ledgers = shopLedgers(counter!);
    expect(ledgers.length).toBe(1);
    const ledger = ledgers[0];
    expect(ledger.userData.kind).toBe("shop-ledger");
    expect(ledger.userData.part).toBe("ledger");
    expect(ledger.userData.mode).toBe("PAPER");
    expect(ledger.parent?.name).toBe("shop-counter");
    ...
    expect(mops[0].userData.part).toBe("mop");
    expect(dustpans[0].userData.part).toBe("dustpan");
    expect(soaps[0].userData.part).toBe("soap");
    ...
    for (const other of neighbors) {
      expect(other).toBeTruthy();
      const dx = ledger.position.x - other.position.x;
      const dz = ledger.position.z - other.position.z;
      expect(Math.hypot(dx, dz)).toBeGreaterThan(0.12);
    }

    const colors = hexes(ledger);
    expect(colors.length).toBeGreaterThan(0);
    expect(colors.every((c) => c === WOOD || c === CREAM)).toBe(true);
    expect(colors.some((c) => c === WOOD)).toBe(true);
    expect(colors.some((c) => c === CREAM)).toBe(true);
    ...
    ledger.traverse((obj) => {
      ...
      if (mesh.isMesh) {
        boxes += 1;
        expect(mesh.geometry.type).toBe("BoxGeometry");
        expect(mesh.userData.kind).toBe("shop-ledger");
        expect(mesh.userData.part).toBe("ledger");
        expect(mesh.userData.mode).toBe("PAPER");
        const box = mesh.geometry as THREE.BoxGeometry;
        expect(Math.max(box.parameters.width, box.parameters.height, box.parameters.depth)).toBeLessThan(
          0.15,
...
describe("shop PAPER kraft inkwell", () => {
  it("puts one tiny kraft PAPER inkwell on the shop counter, ledger and soap remain", () => {
    const scene = new THREE.Scene();
    const interior = makeInteriorScene();
    scene.add(interior);
    dressShop(scene);
    ...
    const inkwells = shopInkwells(counter!);
    expect(inkwells.length).toBe(1);
    const inkwell = inkwells[0];
    expect(inkwell.userData.kind).toBe("shop-inkwell");
    expect(inkwell.userData.part).toBe("inkwell");
    expect(inkwell.userData.mode).toBe("PAPER");
    expect(inkwell.parent?.name).toBe("shop-counter");
    ...
    expect(dustpans[0].userData.part).toBe("dustpan");
    expect(soaps[0].userData.part).toBe("soap");
    expect(ledgers[0].userData.part).toBe("ledger");
    ...
    for (const other of neighbors) {
      expect(other).toBeTruthy();
      const dx = inkwell.position.x - other.position.x;
      const dz = inkwell.position.z - other.position.z;
      expect(Math.hypot(dx, dz)).toBeGreaterThan(0.12);
    }

    const colors = hexes(inkwell);
    expect(colors.length).toBeGreaterThan(0);
    expect(colors.every((c) => c === WOOD || c === CREAM)).toBe(true);
    expect(colors.some((c) => c === WOOD)).toBe(true);
    expect(colors.some((c) => c === CREAM)).toBe(true);
    ...
    let boxes = 0;
    inkwell.traverse((obj) => {
      const mesh = obj as THREE.Mesh;
      if (mesh.isMesh) {
        boxes += 1;
        expect(mesh.geometry.type).toBe("BoxGeometry");
        expect(mesh.userData.kind).toBe("shop-inkwell");
        expect(mesh.userData.part).toBe("inkwell");
        expect(mesh.userData.mode).toBe("PAPER");
        const box = mesh.geometry as THREE.BoxGeometry;
        expect(Math.max(box.parameters.width, box.parameters.height, box.parameters.depth)).toBeLessThan(
          0.15,
...
describe("shop PAPER kraft ruler", () => {
  it("puts one tiny kraft PAPER ruler on the shop counter, inkwell and ledger remain", () => {
    const scene = new THREE.Scene();
    const interior = makeInteriorScene();
    scene.add(interior);
    dressShop(scene);
    ...
    const rulers = shopRulers(counter!);
    expect(rulers.length).toBe(1);
    const ruler = rulers[0];
    expect(ruler.userData.kind).toBe("shop-ruler");
    expect(ruler.userData.part).toBe("ruler");
    expect(ruler.userData.mode).toBe("PAPER");
    expect(ruler.parent?.name).toBe("shop-counter");
    ...
    expect(soaps[0].userData.part).toBe("soap");
    expect(ledgers[0].userData.part).toBe("ledger");
    expect(inkwells[0].userData.part).toBe("inkwell");
    ...
    for (const other of neighbors) {
      expect(other).toBeTruthy();
      const dx = ruler.position.x - other.position.x;
      const dz = ruler.position.z - other.position.z;
      expect(Math.hypot(dx, dz)).toBeGreaterThan(0.12);
    }

    const colors = hexes(ruler);
    expect(colors.length).toBeGreaterThan(0);
    expect(colors.every((c) => c === WOOD || c === CREAM)).toBe(true);
    expect(colors.some((c) => c === WOOD)).toBe(true);
    expect(colors.some((c) => c === CREAM)).toBe(true);
    ...
    ruler.traverse((obj) => {
      ...
      if (mesh.isMesh) {
        boxes += 1;
        expect(mesh.geometry.type).toBe("BoxGeometry");
        expect(mesh.userData.kind).toBe("shop-ruler");
        expect(mesh.userData.part).toBe("ruler");
        expect(mesh.userData.mode).toBe("PAPER");
        const box = mesh.geometry as THREE.BoxGeometry;
        expect(Math.max(box.parameters.width, box.parameters.height, box.parameters.depth)).toBeLessThan(
          0.15,
...
describe("shop PAPER kraft envelope", () => {
  it("puts one tiny kraft PAPER envelope on the shop counter, ruler and inkwell remain", () => {
    const scene = new THREE.Scene();
    const interior = makeInteriorScene();
    scene.add(interior);
    dressShop(scene);
    ...
    const envelopes = shopEnvelopes(counter!);
    expect(envelopes.length).toBe(1);
    const envelope = envelopes[0];
    expect(envelope.userData.kind).toBe("shop-envelope");
    expect(envelope.userData.part).toBe("envelope");
    expect(envelope.userData.mode).toBe("PAPER");
    expect(envelope.parent?.name).toBe("shop-counter");
    ...
    expect(ledgers[0].userData.part).toBe("ledger");
    expect(inkwells[0].userData.part).toBe("inkwell");
    expect(rulers[0].userData.part).toBe("ruler");
    ...
    for (const other of neighbors) {
      expect(other).toBeTruthy();
      const dx = envelope.position.x - other.position.x;
      const dz = envelope.position.z - other.position.z;
      expect(Math.hypot(dx, dz)).toBeGreaterThan(0.12);
    }

    const colors = hexes(envelope);
    expect(colors.length).toBeGreaterThan(0);
    expect(colors.every((c) => c === WOOD || c === CREAM)).toBe(true);
    expect(colors.some((c) => c === WOOD)).toBe(true);
    expect(colors.some((c) => c === CREAM)).toBe(true);
    ...
    let boxes = 0;
    envelope.traverse((obj) => {
      const mesh = obj as THREE.Mesh;
      if (mesh.isMesh) {
        boxes += 1;
        expect(mesh.geometry.type).toBe("BoxGeometry");
        expect(mesh.userData.kind).toBe("shop-envelope");
        expect(mesh.userData.part).toBe("envelope");
        expect(mesh.userData.mode).toBe("PAPER");
        const box = mesh.geometry as THREE.BoxGeometry;
        expect(Math.max(box.parameters.width, box.parameters.height, box.parameters.depth)).toBeLessThan(
          0.15,
...
describe("shop PAPER kraft wafer", () => {
  it("puts one tiny kraft PAPER wafer on the shop counter, envelope and ruler remain", () => {
    const scene = new THREE.Scene();
    const interior = makeInteriorScene();
    scene.add(interior);
    dressShop(scene);
    ...
    const wafers = shopWafers(counter!);
    expect(wafers.length).toBe(1);
    const wafer = wafers[0];
    expect(wafer.userData.kind).toBe("shop-wafer");
    expect(wafer.userData.part).toBe("wafer");
    expect(wafer.userData.mode).toBe("PAPER");
    expect(wafer.parent?.name).toBe("shop-counter");
    ...
    expect(inkwells[0].userData.part).toBe("inkwell");
    expect(rulers[0].userData.part).toBe("ruler");
    expect(envelopes[0].userData.part).toBe("envelope");
    ...
    for (const other of neighbors) {
      expect(other).toBeTruthy();
      const dx = wafer.position.x - other.position.x;
      const dz = wafer.position.z - other.position.z;
      expect(Math.hypot(dx, dz)).toBeGreaterThan(0.12);
    }

    const colors = hexes(wafer);
    expect(colors.length).toBeGreaterThan(0);
    expect(colors.every((c) => c === WOOD || c === CREAM)).toBe(true);
    expect(colors.some((c) => c === WOOD)).toBe(true);
    expect(colors.some((c) => c === CREAM)).toBe(true);
    ...
    wafer.traverse((obj) => {
      ...
      if (mesh.isMesh) {
        boxes += 1;
        expect(mesh.geometry.type).toBe("BoxGeometry");
        expect(mesh.userData.kind).toBe("shop-wafer");
        expect(mesh.userData.part).toBe("wafer");
        expect(mesh.userData.mode).toBe("PAPER");
        const box = mesh.geometry as THREE.BoxGeometry;
        expect(Math.max(box.parameters.width, box.parameters.height, box.parameters.depth)).toBeLessThan(
          0.15,
