...
describe("house-shop PAPER counter tin", () => {
  it("sits a tiny kraft PAPER tin on the house-shop counter", () => {
    const scene = new THREE.Scene();
    const interior = makeInteriorScene();
    scene.add(interior);
    dressHouseShop(scene);
    ...
    const tins: THREE.Object3D[] = [];
    dress!.traverse((obj) => {
      if (obj.userData?.kind === "house-shop-tin" && obj.name === "house-shop-tin") {
        tins.push(obj);
      }
    });
    expect(tins.length).toBe(1);

    const tin = tins[0];
    expect(tin.userData.kind).toBe("house-shop-tin");
    expect(tin.userData.part).toBe("tin");
    expect(tin.userData.mode).toBe("PAPER");
    // Counter top is ~1.12; tin sits on it, not hanging with the bell.
    expect(tin.position.y).toBeGreaterThan(1.0);
    expect(tin.position.y).toBeLessThan(1.25);
    expect(Math.abs(tin.position.x)).toBeLessThan(1.2);
    expect(tin.position.z).toBeGreaterThan(0.1);
    expect(tin.position.z).toBeLessThan(0.9);

    const pan = dress!.getObjectByName("house-shop-pan");
    const pot = dress!.getObjectByName("house-shop-pot");
    const jug = dress!.getObjectByName("house-shop-jug");
    const kettle = dress!.getObjectByName("house-shop-kettle");
    const jar = dress!.getObjectByName("house-shop-jar");
    ...
    expect(bell).toBeTruthy();
    expect(cup).toBeTruthy();
    const panOffset = Math.hypot(
      tin.position.x - pan!.position.x,
      tin.position.z - pan!.position.z,
    );
    const potOffset = Math.hypot(
      tin.position.x - pot!.position.x,
      tin.position.z - pot!.position.z,
    );
    const jugOffset = Math.hypot(
      tin.position.x - jug!.position.x,
      tin.position.z - jug!.position.z,
    );
    const kettleOffset = Math.hypot(
      tin.position.x - kettle!.position.x,
      tin.position.z - kettle!.position.z,
    );
    const jarOffset = Math.hypot(
      tin.position.x - jar!.position.x,
      tin.position.z - jar!.position.z,
    );
    const padOffset = Math.hypot(
      tin.position.x - pad!.position.x,
      tin.position.z - pad!.position.z,
    );
    const bellOffset = Math.hypot(
      tin.position.x - bell!.position.x,
      tin.position.z - bell!.position.z,
    );
    const cupOffset = Math.hypot(
      tin.position.x - cup!.position.x,
      tin.position.z - cup!.position.z,
    );
    expect(panOffset).toBeGreaterThan(0.25);
    expect(potOffset).toBeGreaterThan(0.25);
    expect(jugOffset).toBeGreaterThan(0.25);
    expect(kettleOffset).toBeGreaterThan(0.25);
    expect(jarOffset).toBeGreaterThan(0.25);
    ...
    tin.getWorldPosition(tinPos);
    const xzOffset = (other: THREE.Object3D) => {
      const otherPos = new THREE.Vector3();
      other.getWorldPosition(otherPos);
      return Math.hypot(tinPos.x - otherPos.x, tinPos.z - otherPos.z);
    };
    const neighbors: THREE.Object3D[] = [];
    dress!.traverse((obj) => {
      if (obj.userData?.part === "saucer") {
        neighbors.push(obj);
      }
    });
    ...
    tin.traverse((obj) => {
      ...
      if (mesh.isMesh) {
        expect(mesh.geometry.type).toBe("BoxGeometry");
        expect(mesh.userData.kind).toBe("house-shop-tin");
        expect(mesh.userData.mode).toBe("PAPER");
      }
    });
  });
...
describe("house-shop PAPER tiny kettle", () => {
  it("sits a tiny kraft PAPER kettle on the house-shop counter", () => {
    const scene = new THREE.Scene();
    const interior = makeInteriorScene();
    scene.add(interior);
    dressHouseShop(scene);
    ...
    const kettle = kettles[0];
    expect(kettle.userData.kind).toBe("house-shop-tiny-kettle");
    expect(kettle.userData.part).toBe("kettle");
    expect(kettle.userData.mode).toBe("PAPER");
    // Counter top is ~1.12; tiny kettle sits on it, not hanging with the bell.
    expect(kettle.position.y).toBeGreaterThan(1.0);
    expect(kettle.position.y).toBeLessThan(1.25);
    expect(Math.abs(kettle.position.x)).toBeLessThan(1.2);
    expect(kettle.position.z).toBeGreaterThan(0.1);
    ...
    const pan = dress!.getObjectByName("house-shop-pan");
    const tin = dress!.getObjectByName("house-shop-tin");
    const pot = dress!.getObjectByName("house-shop-pot");
    const jug = dress!.getObjectByName("house-shop-jug");
    const cup = dress!.getObjectByName("house-shop-cup");
    expect(pan).toBeTruthy();
    ...
    expect(cup).toBeTruthy();
    const panOffset = Math.hypot(
      kettle.position.x - pan!.position.x,
      kettle.position.z - pan!.position.z,
    );
    const tinOffset = Math.hypot(
      kettle.position.x - tin!.position.x,
      kettle.position.z - tin!.position.z,
    );
    const potOffset = Math.hypot(
      kettle.position.x - pot!.position.x,
      kettle.position.z - pot!.position.z,
    );
    const jugOffset = Math.hypot(
      kettle.position.x - jug!.position.x,
      kettle.position.z - jug!.position.z,
    );
    const cupOffset = Math.hypot(
      kettle.position.x - cup!.position.x,
      kettle.position.z - cup!.position.z,
    );
    expect(panOffset).toBeGreaterThan(0.25);
    expect(tinOffset).toBeGreaterThan(0.25);
    expect(potOffset).toBeGreaterThan(0.25);
    expect(jugOffset).toBeGreaterThan(0.25);
    expect(cupOffset).toBeGreaterThan(0.25);

    const kettlePos = new THREE.Vector3();
    kettle.getWorldPosition(kettlePos);
    const xzOffset = (other: THREE.Object3D) => {
      const otherPos = new THREE.Vector3();
      other.getWorldPosition(otherPos);
      return Math.hypot(kettlePos.x - otherPos.x, kettlePos.z - otherPos.z);
    };
    const neighbors: THREE.Object3D[] = [];
    dress!.traverse((obj) => {
      if (
    ...
    kettle.traverse((obj) => {
      ...
      if (mesh.isMesh) {
        expect(mesh.geometry.type).toBe("BoxGeometry");
        expect(mesh.userData.kind).toBe("house-shop-tiny-kettle");
        expect(mesh.userData.mode).toBe("PAPER");
      }
    });
  });
...
describe("house-shop PAPER tiny colander", () => {
  it("sits a tiny kraft PAPER colander on the house-shop counter", () => {
    const scene = new THREE.Scene();
    const interior = makeInteriorScene();
    scene.add(interior);
    dressHouseShop(scene);
    ...
    const colanders: THREE.Object3D[] = [];
    dress!.traverse((obj) => {
      if (obj.userData?.part === "colander") {
        colanders.push(obj);
      }
    });
    expect(colanders.length).toBe(1);

    const colander = colanders[0];
    expect(colander.userData.kind).toBe("house-shop-colander");
    expect(colander.userData.part).toBe("colander");
    expect(colander.userData.mode).toBe("PAPER");
    // Counter top is ~1.12; colander sits on it, not hanging with the bell.
    expect(colander.position.y).toBeGreaterThan(1.0);
    expect(colander.position.y).toBeLessThan(1.25);
    expect(Math.abs(colander.position.x)).toBeLessThan(1.2);
    expect(colander.position.z).toBeGreaterThan(0.1);
    expect(colander.position.z).toBeLessThan(0.9);

    const kettles: THREE.Object3D[] = [];
    dress!.traverse((obj) => {
      if (obj.userData?.part === "kettle") kettles.push(obj);
    });
    const kettle = kettles[0];
    const pan = dress!.getObjectByName("house-shop-pan");
    const tin = dress!.getObjectByName("house-shop-tin");
    ...
    expect(cup).toBeTruthy();
    const kettleOffset = Math.hypot(
      colander.position.x - kettle!.position.x,
      colander.position.z - kettle!.position.z,
    );
    const panOffset = Math.hypot(
      colander.position.x - pan!.position.x,
      colander.position.z - pan!.position.z,
    );
    const tinOffset = Math.hypot(
      colander.position.x - tin!.position.x,
      colander.position.z - tin!.position.z,
    );
    const potOffset = Math.hypot(
      colander.position.x - pot!.position.x,
      colander.position.z - pot!.position.z,
    );
    const jugOffset = Math.hypot(
      colander.position.x - jug!.position.x,
      colander.position.z - jug!.position.z,
    );
    const cupOffset = Math.hypot(
      colander.position.x - cup!.position.x,
      colander.position.z - cup!.position.z,
    );
    expect(kettleOffset).toBeGreaterThan(0.25);
    expect(panOffset).toBeGreaterThan(0.25);
    expect(tinOffset).toBeGreaterThan(0.25);
    ...
    const colanderPos = new THREE.Vector3();
    colander.getWorldPosition(colanderPos);
    const xzOffset = (other: THREE.Object3D) => {
      const otherPos = new THREE.Vector3();
      other.getWorldPosition(otherPos);
      return Math.hypot(colanderPos.x - otherPos.x, colanderPos.z - otherPos.z);
    };
    const neighbors: THREE.Object3D[] = [];
    dress!.traverse((obj) => {
      if (
    ...
    colander.traverse((obj) => {
      const mesh = obj as THREE.Mesh;
      if (mesh.isMesh) {
        expect(mesh.geometry.type).toBe("BoxGeometry");
        expect(mesh.userData.kind).toBe("house-shop-colander");
        expect(mesh.userData.mode).toBe("PAPER");
      }
    });
  });
