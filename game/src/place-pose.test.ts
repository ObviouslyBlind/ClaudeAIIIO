...
describe("place ghost", () => {
  it("draws a green outline and yaws while the rotate action is held", () => {
    ...
    const fill = scene.getObjectByName("place-ghost-fill");
    expect(fill).toBeTruthy();
    expect(fill.position.y).toBeGreaterThan(0.4);
    expect((fill.material.color.getHex() as number)).toBe(PLACE_GHOST_OK);
