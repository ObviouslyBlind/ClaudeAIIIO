...
describe("farm PAPER kraft fork", () => {
  it("leans one tiny kraft PAPER fork by the workbench; flail yoke shears trowel spade ladle scythe sickle hoe rake broom seed lid mug egg pail lantern remain", () => {
    const scene = new THREE.Scene();
    dressFarm(scene);
    ...
    const forks: THREE.Object3D[] = [];
    dress!.traverse((obj) => {
      if (obj.userData?.part === "fork" && obj.name === "farm-paper-fork") forks.push(obj);
    });
    expect(forks.length).toBe(1);
    const paperFork = forks[0];
    expect(paperFork.userData.part).toBe("fork");
    expect(paperFork.userData.mode).toBe("PAPER");
    expect(Math.abs(paperFork.rotation.z) > 0.15 || Math.abs(paperFork.rotation.x) > 0.15).toBe(true);
    ...
    expect(paperFork.position.y).toBeLessThan(0.5);
    const toBench = Math.hypot(paperFork.position.x - -3.28, paperFork.position.z - -0.15);
    expect(toBench).toBeGreaterThan(0.2);
    expect(toBench).toBeLessThan(0.8);

    expect(Math.hypot(paperFork.position.x - paperFlail.position.x, paperFork.position.z - paperFlail.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperFork.position.x - paperYoke.position.x, paperFork.position.z - paperYoke.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperFork.position.x - paperShears.position.x, paperFork.position.z - paperShears.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperFork.position.x - paperTrowel.position.x, paperFork.position.z - paperTrowel.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperFork.position.x - paperSpade.position.x, paperFork.position.z - paperSpade.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperFork.position.x - paperLadle.position.x, paperFork.position.z - paperLadle.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperFork.position.x - paperScythe.position.x, paperFork.position.z - paperScythe.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperFork.position.x - paperSickle.position.x, paperFork.position.z - paperSickle.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperFork.position.x - paperHoe.position.x, paperFork.position.z - paperHoe.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperFork.position.x - paperRake.position.x, paperFork.position.z - paperRake.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperFork.position.x - broom.position.x, paperFork.position.z - broom.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperFork.position.x - seed.position.x, paperFork.position.z - seed.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperFork.position.x - mug.position.x, paperFork.position.z - mug.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperFork.position.x - egg.position.x, paperFork.position.z - egg.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperFork.position.x - tablePail.position.x, paperFork.position.z - tablePail.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperFork.position.x - lantern.position.x, paperFork.position.z - lantern.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperFork.position.x - lid.position.x, paperFork.position.z - lid.position.z)).toBeGreaterThan(4);

    const colors = hexes(paperFork);
    expect(colors.some((c) => c === KRAFT)).toBe(true);
    expect(colors.every((c) => [KRAFT, WOOD, WOOD_DARK, HANDLE].includes(c))).toBe(true);

    let boxes = 0;
    paperFork.traverse((obj) => {
      const mesh = obj as THREE.Mesh;
      if (mesh.isMesh) {
        boxes += 1;
        expect(mesh.geometry.type).toBe("BoxGeometry");
...
describe("farm PAPER kraft mattock", () => {
  it("leans one tiny kraft PAPER mattock by the workbench; harrow fork flail remain", () => {
    const scene = new THREE.Scene();
    dressFarm(scene);
    ...
    const mattocks: THREE.Object3D[] = [];
    dress!.traverse((obj) => {
      if (obj.userData?.part === "mattock" && obj.name === "farm-mattock") mattocks.push(obj);
    });
    expect(mattocks.length).toBe(1);
    const paperMattock = mattocks[0];
    expect(paperMattock.userData.part).toBe("mattock");
    expect(paperMattock.userData.mode).toBe("PAPER");
    expect(Math.abs(paperMattock.rotation.z) > 0.15 || Math.abs(paperMattock.rotation.x) > 0.15).toBe(true);

    const paperHarrow = dress!.getObjectByName("farm-harrow")!;
    const paperFork = dress!.getObjectByName("farm-paper-fork")!;
    const paperFlail = dress!.getObjectByName("farm-flail")!;
    const paperYoke = dress!.getObjectByName("farm-yoke")!;
    const paperShears = dress!.getObjectByName("farm-shears")!;
    ...
    expect(paperMattock.position.y).toBeLessThan(0.5);
    const toBench = Math.hypot(paperMattock.position.x - -3.28, paperMattock.position.z - -0.15);
    expect(toBench).toBeGreaterThan(0.2);
    expect(toBench).toBeLessThan(0.8);

    expect(Math.hypot(paperMattock.position.x - paperHarrow.position.x, paperMattock.position.z - paperHarrow.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperMattock.position.x - paperFork.position.x, paperMattock.position.z - paperFork.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperMattock.position.x - paperFlail.position.x, paperMattock.position.z - paperFlail.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperMattock.position.x - paperYoke.position.x, paperMattock.position.z - paperYoke.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperMattock.position.x - paperShears.position.x, paperMattock.position.z - paperShears.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperMattock.position.x - paperTrowel.position.x, paperMattock.position.z - paperTrowel.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperMattock.position.x - paperSpade.position.x, paperMattock.position.z - paperSpade.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperMattock.position.x - paperLadle.position.x, paperMattock.position.z - paperLadle.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperMattock.position.x - paperScythe.position.x, paperMattock.position.z - paperScythe.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperMattock.position.x - paperSickle.position.x, paperMattock.position.z - paperSickle.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperMattock.position.x - paperHoe.position.x, paperMattock.position.z - paperHoe.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperMattock.position.x - paperRake.position.x, paperMattock.position.z - paperRake.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperMattock.position.x - broom.position.x, paperMattock.position.z - broom.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperMattock.position.x - seed.position.x, paperMattock.position.z - seed.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperMattock.position.x - mug.position.x, paperMattock.position.z - mug.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperMattock.position.x - egg.position.x, paperMattock.position.z - egg.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperMattock.position.x - tablePail.position.x, paperMattock.position.z - tablePail.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperMattock.position.x - lantern.position.x, paperMattock.position.z - lantern.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperMattock.position.x - lid.position.x, paperMattock.position.z - lid.position.z)).toBeGreaterThan(4);

    const colors = hexes(paperMattock);
    expect(colors.some((c) => c === KRAFT)).toBe(true);
    expect(colors.every((c) => [KRAFT, WOOD, WOOD_DARK, HANDLE].includes(c))).toBe(true);

    let boxes = 0;
    paperMattock.traverse((obj) => {
      const mesh = obj as THREE.Mesh;
      if (mesh.isMesh) {
        boxes += 1;
        expect(mesh.geometry.type).toBe("BoxGeometry");
        expect(mesh.userData.part).toBe("mattock");
        expect(mesh.userData.mode).toBe("PAPER");
        const geo = mesh.geometry as THREE.BoxGeometry;
        expect(geo.parameters.width).toBeLessThan(0.16);
        expect(geo.parameters.height).toBeLessThan(0.4);
...
describe("farm PAPER kraft hod", () => {
  it("leans one tiny kraft PAPER hod by the workbench; mattock harrow fork flail remain", () => {
    const scene = new THREE.Scene();
    dressFarm(scene);
    ...
    const hods: THREE.Object3D[] = [];
    dress!.traverse((obj) => {
      if (obj.userData?.part === "hod" && obj.name === "farm-hod") hods.push(obj);
    });
    expect(hods.length).toBe(1);
    const paperHod = hods[0];
    expect(paperHod.userData.part).toBe("hod");
    expect(paperHod.userData.mode).toBe("PAPER");
    expect(Math.abs(paperHod.rotation.z) > 0.15 || Math.abs(paperHod.rotation.x) > 0.15).toBe(true);

    const paperMattock = dress!.getObjectByName("farm-mattock")!;
    const paperHarrow = dress!.getObjectByName("farm-harrow")!;
    const paperFork = dress!.getObjectByName("farm-paper-fork")!;
    const paperFlail = dress!.getObjectByName("farm-flail")!;
    const paperYoke = dress!.getObjectByName("farm-yoke")!;
    ...
    const tablePail = dress!.getObjectByName("farm-table-pail")!;
    const lantern = dress!.getObjectByName("farm-lantern")!;
    expect(paperMattock).toBeTruthy();
    expect(paperHarrow).toBeTruthy();
    expect(paperFork).toBeTruthy();
    expect(paperFlail).toBeTruthy();
    expect(paperYoke).toBeTruthy();
    ...
    expect(paperHod.position.y).toBeLessThan(0.5);
    const toBench = Math.hypot(paperHod.position.x - -3.28, paperHod.position.z - -0.15);
    expect(toBench).toBeGreaterThan(0.2);
    expect(toBench).toBeLessThan(0.8);

    expect(Math.hypot(paperHod.position.x - paperMattock.position.x, paperHod.position.z - paperMattock.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperHod.position.x - paperHarrow.position.x, paperHod.position.z - paperHarrow.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperHod.position.x - paperFork.position.x, paperHod.position.z - paperFork.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperHod.position.x - paperFlail.position.x, paperHod.position.z - paperFlail.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperHod.position.x - paperYoke.position.x, paperHod.position.z - paperYoke.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperHod.position.x - paperShears.position.x, paperHod.position.z - paperShears.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperHod.position.x - paperTrowel.position.x, paperHod.position.z - paperTrowel.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperHod.position.x - paperSpade.position.x, paperHod.position.z - paperSpade.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperHod.position.x - paperLadle.position.x, paperHod.position.z - paperLadle.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperHod.position.x - paperScythe.position.x, paperHod.position.z - paperScythe.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperHod.position.x - paperSickle.position.x, paperHod.position.z - paperSickle.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperHod.position.x - paperHoe.position.x, paperHod.position.z - paperHoe.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperHod.position.x - paperRake.position.x, paperHod.position.z - paperRake.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperHod.position.x - broom.position.x, paperHod.position.z - broom.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperHod.position.x - seed.position.x, paperHod.position.z - seed.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperHod.position.x - mug.position.x, paperHod.position.z - mug.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperHod.position.x - egg.position.x, paperHod.position.z - egg.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperHod.position.x - tablePail.position.x, paperHod.position.z - tablePail.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperHod.position.x - lantern.position.x, paperHod.position.z - lantern.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperHod.position.x - lid.position.x, paperHod.position.z - lid.position.z)).toBeGreaterThan(4);

    const colors = hexes(paperHod);
    expect(colors.some((c) => c === KRAFT)).toBe(true);
    expect(colors.every((c) => [KRAFT, WOOD, WOOD_DARK, HANDLE].includes(c))).toBe(true);

    let boxes = 0;
    paperHod.traverse((obj) => {
      const mesh = obj as THREE.Mesh;
      if (mesh.isMesh) {
        boxes += 1;
        expect(mesh.geometry.type).toBe("BoxGeometry");
        expect(mesh.userData.part).toBe("hod");
        expect(mesh.userData.mode).toBe("PAPER");
        const geo = mesh.geometry as THREE.BoxGeometry;
        expect(geo.parameters.width).toBeLessThan(0.16);
        expect(geo.parameters.height).toBeLessThan(0.4);
...
describe("farm PAPER kraft cask", () => {
  it("leans one tiny kraft PAPER cask by the workbench; hod mattock harrow fork flail remain", () => {
    const scene = new THREE.Scene();
    dressFarm(scene);
    ...
    const casks: THREE.Object3D[] = [];
    dress!.traverse((obj) => {
      if (obj.userData?.part === "cask" && obj.name === "farm-cask") casks.push(obj);
    });
    expect(casks.length).toBe(1);
    const paperCask = casks[0];
    expect(paperCask.userData.part).toBe("cask");
    expect(paperCask.userData.mode).toBe("PAPER");
    expect(Math.abs(paperCask.rotation.z) > 0.15 || Math.abs(paperCask.rotation.x) > 0.15).toBe(true);

    const paperHod = dress!.getObjectByName("farm-hod")!;
    const paperMattock = dress!.getObjectByName("farm-mattock")!;
    const paperHarrow = dress!.getObjectByName("farm-harrow")!;
    const paperFork = dress!.getObjectByName("farm-paper-fork")!;
    const paperFlail = dress!.getObjectByName("farm-flail")!;
    ...
    expect(paperCask.position.y).toBeLessThan(0.5);
    const toBench = Math.hypot(paperCask.position.x - -3.28, paperCask.position.z - -0.15);
    expect(toBench).toBeGreaterThan(0.2);
    expect(toBench).toBeLessThan(0.8);

    expect(Math.hypot(paperCask.position.x - paperHod.position.x, paperCask.position.z - paperHod.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperCask.position.x - paperMattock.position.x, paperCask.position.z - paperMattock.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperCask.position.x - paperHarrow.position.x, paperCask.position.z - paperHarrow.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperCask.position.x - paperFork.position.x, paperCask.position.z - paperFork.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperCask.position.x - paperFlail.position.x, paperCask.position.z - paperFlail.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperCask.position.x - paperYoke.position.x, paperCask.position.z - paperYoke.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperCask.position.x - paperShears.position.x, paperCask.position.z - paperShears.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperCask.position.x - paperTrowel.position.x, paperCask.position.z - paperTrowel.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperCask.position.x - paperSpade.position.x, paperCask.position.z - paperSpade.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperCask.position.x - paperLadle.position.x, paperCask.position.z - paperLadle.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperCask.position.x - paperScythe.position.x, paperCask.position.z - paperScythe.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperCask.position.x - paperSickle.position.x, paperCask.position.z - paperSickle.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperCask.position.x - paperHoe.position.x, paperCask.position.z - paperHoe.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperCask.position.x - paperRake.position.x, paperCask.position.z - paperRake.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperCask.position.x - broom.position.x, paperCask.position.z - broom.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperCask.position.x - seed.position.x, paperCask.position.z - seed.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperCask.position.x - mug.position.x, paperCask.position.z - mug.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperCask.position.x - egg.position.x, paperCask.position.z - egg.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperCask.position.x - tablePail.position.x, paperCask.position.z - tablePail.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperCask.position.x - lantern.position.x, paperCask.position.z - lantern.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperCask.position.x - lid.position.x, paperCask.position.z - lid.position.z)).toBeGreaterThan(4);

    const colors = hexes(paperCask);
    expect(colors.some((c) => c === KRAFT)).toBe(true);
    expect(colors.every((c) => [KRAFT, WOOD, WOOD_DARK, HANDLE].includes(c))).toBe(true);

    let boxes = 0;
    paperCask.traverse((obj) => {
      const mesh = obj as THREE.Mesh;
      if (mesh.isMesh) {
        boxes += 1;
        expect(mesh.geometry.type).toBe("BoxGeometry");
        expect(mesh.userData.part).toBe("cask");
        expect(mesh.userData.mode).toBe("PAPER");
        const geo = mesh.geometry as THREE.BoxGeometry;
        expect(geo.parameters.width).toBeLessThan(0.16);
        expect(geo.parameters.height).toBeLessThan(0.4);
...
describe("farm PAPER kraft winnow", () => {
  it("leans one tiny kraft PAPER winnow by the workbench; cask hod mattock harrow fork flail remain", () => {
    const scene = new THREE.Scene();
    dressFarm(scene);
    ...
    const winnows: THREE.Object3D[] = [];
    dress!.traverse((obj) => {
      if (obj.userData?.part === "winnow" && obj.name === "farm-winnow") winnows.push(obj);
    });
    expect(winnows.length).toBe(1);
    const paperWinnow = winnows[0];
    expect(paperWinnow.userData.part).toBe("winnow");
    expect(paperWinnow.userData.mode).toBe("PAPER");
    expect(Math.abs(paperWinnow.rotation.z) > 0.15 || Math.abs(paperWinnow.rotation.x) > 0.15).toBe(true);

    const paperCask = dress!.getObjectByName("farm-cask")!;
    const paperHod = dress!.getObjectByName("farm-hod")!;
    const paperMattock = dress!.getObjectByName("farm-mattock")!;
    const paperHarrow = dress!.getObjectByName("farm-harrow")!;
    const paperFork = dress!.getObjectByName("farm-paper-fork")!;
    ...
    expect(paperWinnow.position.y).toBeLessThan(0.5);
    const toBench = Math.hypot(paperWinnow.position.x - -3.28, paperWinnow.position.z - -0.15);
    expect(toBench).toBeGreaterThan(0.2);
    expect(toBench).toBeLessThan(0.8);

    expect(Math.hypot(paperWinnow.position.x - paperCask.position.x, paperWinnow.position.z - paperCask.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperWinnow.position.x - paperHod.position.x, paperWinnow.position.z - paperHod.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperWinnow.position.x - paperMattock.position.x, paperWinnow.position.z - paperMattock.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperWinnow.position.x - paperHarrow.position.x, paperWinnow.position.z - paperHarrow.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperWinnow.position.x - paperFork.position.x, paperWinnow.position.z - paperFork.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperWinnow.position.x - paperFlail.position.x, paperWinnow.position.z - paperFlail.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperWinnow.position.x - paperYoke.position.x, paperWinnow.position.z - paperYoke.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperWinnow.position.x - paperShears.position.x, paperWinnow.position.z - paperShears.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperWinnow.position.x - paperTrowel.position.x, paperWinnow.position.z - paperTrowel.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperWinnow.position.x - paperSpade.position.x, paperWinnow.position.z - paperSpade.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperWinnow.position.x - paperLadle.position.x, paperWinnow.position.z - paperLadle.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperWinnow.position.x - paperScythe.position.x, paperWinnow.position.z - paperScythe.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperWinnow.position.x - paperSickle.position.x, paperWinnow.position.z - paperSickle.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperWinnow.position.x - paperHoe.position.x, paperWinnow.position.z - paperHoe.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperWinnow.position.x - paperRake.position.x, paperWinnow.position.z - paperRake.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperWinnow.position.x - broom.position.x, paperWinnow.position.z - broom.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperWinnow.position.x - seed.position.x, paperWinnow.position.z - seed.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperWinnow.position.x - mug.position.x, paperWinnow.position.z - mug.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperWinnow.position.x - egg.position.x, paperWinnow.position.z - egg.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperWinnow.position.x - tablePail.position.x, paperWinnow.position.z - tablePail.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperWinnow.position.x - lantern.position.x, paperWinnow.position.z - lantern.position.z)).toBeGreaterThan(0.2);
    expect(Math.hypot(paperWinnow.position.x - lid.position.x, paperWinnow.position.z - lid.position.z)).toBeGreaterThan(4);

    const colors = hexes(paperWinnow);
    expect(colors.some((c) => c === KRAFT)).toBe(true);
    expect(colors.every((c) => [KRAFT, WOOD, WOOD_DARK, HANDLE].includes(c))).toBe(true);

    let boxes = 0;
    paperWinnow.traverse((obj) => {
      const mesh = obj as THREE.Mesh;
      if (mesh.isMesh) {
        boxes += 1;
        expect(mesh.geometry.type).toBe("BoxGeometry");
        expect(mesh.userData.part).toBe("winnow");
        expect(mesh.userData.mode).toBe("PAPER");
        const geo = mesh.geometry as THREE.BoxGeometry;
        expect(geo.parameters.width).toBeLessThan(0.16);
        expect(geo.parameters.height).toBeLessThan(0.4);
