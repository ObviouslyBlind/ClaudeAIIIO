...
describe("street prop setback", () => {
  ...
  it("sits a tiny kraft PAPER bung on the street-props barrel, lid bolt washer peg hook dipper crank remain", () => {
    const map = createLandBoard();
    const scene = { add(_obj: THREE.Object3D) {} };
    const root = makeStreetProps(map, {
      scene,
    ...
    const cranks: THREE.Object3D[] = [];
    root.traverse((obj) => {
      if (obj.userData?.prop === "hawser-drum") drums.push(obj);
      if (obj.userData?.part === "lid") lids.push(obj);
      if (obj.userData?.part === "bolt") bolts.push(obj);
      if (obj.userData?.part === "washer") washers.push(obj);
      if (obj.userData?.part === "peg") pegs.push(obj);
    ...
    for (const drum of drums) {
      expect(drum.userData.mode).toBe("PAPER");
      ...
      for (const b of bungs) {
        expect(b.userData.part).toBe("bung");
        expect(b.userData.mode === "PAPER" || drum.userData.mode === "PAPER").toBe(true);
        const mesh = b as THREE.Mesh;
        expect(mesh.isMesh).toBe(true);
        expect(mesh.geometry.type).toBe("BoxGeometry");
        const mat = mesh.material as THREE.MeshLambertMaterial;
  ...
  it("sits a tiny kraft PAPER spigot on the village pump, bung lid bolt washer peg hook dipper crank remain", () => {
    const map = createLandBoard();
    const scene = { add(_obj: THREE.Object3D) {} };
    const root = makeStreetProps(map, {
      scene,
    ...
    for (const pump of pumpGroups) {
      ...
      for (const s of spigots) {
        expect(s.userData.part).toBe("spigot");
        expect(s.userData.mode === "PAPER" || pump.userData.mode === "PAPER").toBe(true);
  ...
  it("sits a tiny kraft PAPER marlin on the village pump, fid tap spigot hook peg washer bolt remain", () => {
    const map = createLandBoard();
    const scene = { add(_obj: THREE.Object3D) {} };
    const root = makeStreetProps(map, {
      scene,
    ...
    for (const pump of pumpGroups) {
      expect(pump.userData.mode).toBe("PAPER");
      const marlins: THREE.Object3D[] = [];
      pump.traverse((obj) => {
        if (obj.userData?.part === "marlin") marlins.push(obj);
      });
      expect(marlins.length).toBeGreaterThanOrEqual(1);
      for (const m of marlins) {
        expect(m.userData.part).toBe("marlin");
        expect(m.userData.mode === "PAPER" || pump.userData.mode === "PAPER").toBe(true);
