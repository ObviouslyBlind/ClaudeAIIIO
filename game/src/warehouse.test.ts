...
describe("warehouse PAPER kraft docket", () => {
  it("sits one tiny kraft PAPER docket on the clipboard; tag, ribbon, tack stay", () => {
    const scene = new THREE.Scene();
    const interior = makeInteriorScene();
    scene.add(interior);
    dressWarehouse(scene);
    ...
    const clipboards: THREE.Object3D[] = [];
    dress!.traverse((obj) => {
      if (obj.userData?.kind === "warehouse-docket" && obj.name === "warehouse-docket") {
        dockets.push(obj);
      }
      if (obj.userData?.kind === "warehouse-tag" && obj.name === "warehouse-tag") {
        tags.push(obj);
    ...
    expect(docket.userData.kind).toBe("warehouse-docket");
    expect(docket.userData.mode).toBe("PAPER");
    expect(docket.userData.part).toBe("docket");
    expect(docket.position.y).toBeGreaterThan(1.2);
    expect(docket.position.y).toBeLessThan(2.4);
    const onBack = Math.abs(docket.position.z) > 3.0;
    const onSide = Math.abs(docket.position.x) > 3.5;
    expect(onBack || onSide).toBe(true);
    expect(horizDist(docket, clipboards[0])).toBeLessThan(0.5);
    expect(horizDist(docket, cards[0])).toBeGreaterThan(1.5);
    expect(horizDist(docket, twines[0])).toBeGreaterThan(1.5);
    expect(tags[0].userData.part).toBe("tag");
    expect(ribbons[0].userData.part).toBe("ribbon");
    expect(tacks[0].userData.part).toBe("tack");
    expect(erasers[0].userData.part).toBe("eraser");
    expect(pencils[0].userData.part).toBe("pencil");
    ...
    docket.traverse((obj) => {
      ...
      if (mesh.isMesh) {
        boxes += 1;
        expect(mesh.geometry.type).toBe("BoxGeometry");
        expect(mesh.userData.kind).toBe("warehouse-docket");
        expect(mesh.userData.mode).toBe("PAPER");
      }
    });
    expect(boxes).toBeGreaterThanOrEqual(1);
...
describe("warehouse PAPER kraft label", () => {
  it("sits one tiny kraft PAPER label on the clipboard; docket, tag, ribbon stay", () => {
    const scene = new THREE.Scene();
    const interior = makeInteriorScene();
    scene.add(interior);
    dressWarehouse(scene);
    ...
    const clipboards: THREE.Object3D[] = [];
    dress!.traverse((obj) => {
      if (obj.userData?.kind === "warehouse-label" && obj.name === "warehouse-label") {
        labels.push(obj);
      }
      if (obj.userData?.kind === "warehouse-docket" && obj.name === "warehouse-docket") {
        dockets.push(obj);
    ...
    expect(label.userData.kind).toBe("warehouse-label");
    expect(label.userData.mode).toBe("PAPER");
    expect(label.userData.part).toBe("label");
    expect(label.position.y).toBeGreaterThan(1.2);
    expect(label.position.y).toBeLessThan(2.4);
    const onBack = Math.abs(label.position.z) > 3.0;
    const onSide = Math.abs(label.position.x) > 3.5;
    expect(onBack || onSide).toBe(true);
    expect(horizDist(label, clipboards[0])).toBeLessThan(0.5);
    expect(horizDist(label, cards[0])).toBeGreaterThan(1.5);
    expect(horizDist(label, twines[0])).toBeGreaterThan(1.5);
    expect(dockets[0].userData.part).toBe("docket");
    expect(tags[0].userData.part).toBe("tag");
    expect(ribbons[0].userData.part).toBe("ribbon");
    expect(tacks[0].userData.part).toBe("tack");
    expect(erasers[0].userData.part).toBe("eraser");
    ...
    label.traverse((obj) => {
      ...
      if (mesh.isMesh) {
        boxes += 1;
        expect(mesh.geometry.type).toBe("BoxGeometry");
        expect(mesh.userData.kind).toBe("warehouse-label");
        expect(mesh.userData.mode).toBe("PAPER");
      }
    });
    expect(boxes).toBeGreaterThanOrEqual(1);
...
describe("warehouse PAPER kraft staple", () => {
  it("sits one tiny kraft PAPER staple on the clipboard; label, docket, tag stay", () => {
    const scene = new THREE.Scene();
    const interior = makeInteriorScene();
    scene.add(interior);
    dressWarehouse(scene);
    ...
    const clipboards: THREE.Object3D[] = [];
    dress!.traverse((obj) => {
      if (obj.userData?.kind === "warehouse-staple" && obj.name === "warehouse-staple") {
        staples.push(obj);
      }
      if (obj.userData?.kind === "warehouse-label" && obj.name === "warehouse-label") {
        labels.push(obj);
    ...
    expect(staple.userData.kind).toBe("warehouse-staple");
    expect(staple.userData.mode).toBe("PAPER");
    expect(staple.userData.part).toBe("staple");
    expect(staple.position.y).toBeGreaterThan(1.2);
    expect(staple.position.y).toBeLessThan(2.4);
    const onBack = Math.abs(staple.position.z) > 3.0;
    const onSide = Math.abs(staple.position.x) > 3.5;
    expect(onBack || onSide).toBe(true);
    expect(horizDist(staple, clipboards[0])).toBeLessThan(0.5);
    expect(horizDist(staple, cards[0])).toBeGreaterThan(1.5);
    expect(horizDist(staple, twines[0])).toBeGreaterThan(1.5);
    expect(labels[0].userData.part).toBe("label");
    expect(dockets[0].userData.part).toBe("docket");
    expect(tags[0].userData.part).toBe("tag");
    expect(ribbons[0].userData.part).toBe("ribbon");
    expect(tacks[0].userData.part).toBe("tack");
    ...
    staple.traverse((obj) => {
      ...
      if (mesh.isMesh) {
        boxes += 1;
        expect(mesh.geometry.type).toBe("BoxGeometry");
        expect(mesh.userData.kind).toBe("warehouse-staple");
        expect(mesh.userData.mode).toBe("PAPER");
      }
    });
    expect(boxes).toBeGreaterThanOrEqual(1);
...
describe("warehouse PAPER kraft wax", () => {
  it("sits one tiny kraft PAPER wax stick on the clipboard; staple, label, docket stay", () => {
    const scene = new THREE.Scene();
    const interior = makeInteriorScene();
    scene.add(interior);
    dressWarehouse(scene);
    ...
    const clipboards: THREE.Object3D[] = [];
    dress!.traverse((obj) => {
      if (obj.userData?.kind === "warehouse-wax" && obj.name === "warehouse-wax") {
        waxes.push(obj);
      }
      if (obj.userData?.kind === "warehouse-staple" && obj.name === "warehouse-staple") {
        staples.push(obj);
    ...
    const wax = waxes[0];
    expect(wax.userData.kind).toBe("warehouse-wax");
    expect(wax.userData.mode).toBe("PAPER");
    expect(wax.userData.part).toBe("wax");
    expect(wax.position.y).toBeGreaterThan(1.2);
    expect(wax.position.y).toBeLessThan(2.4);
    const onBack = Math.abs(wax.position.z) > 3.0;
    const onSide = Math.abs(wax.position.x) > 3.5;
    expect(onBack || onSide).toBe(true);
    expect(horizDist(wax, clipboards[0])).toBeLessThan(0.5);
    expect(horizDist(wax, cards[0])).toBeGreaterThan(1.5);
    expect(horizDist(wax, twines[0])).toBeGreaterThan(1.5);
    expect(staples[0].userData.part).toBe("staple");
    expect(labels[0].userData.part).toBe("label");
    expect(dockets[0].userData.part).toBe("docket");
    expect(tags[0].userData.part).toBe("tag");
    expect(ribbons[0].userData.part).toBe("ribbon");
    ...
    wax.traverse((obj) => {
      ...
      if (mesh.isMesh) {
        boxes += 1;
        expect(mesh.geometry.type).toBe("BoxGeometry");
        expect(mesh.userData.kind).toBe("warehouse-wax");
        expect(mesh.userData.mode).toBe("PAPER");
      }
    });
    expect(boxes).toBeGreaterThanOrEqual(1);
...
describe("warehouse PAPER kraft spool", () => {
  it("sits one tiny kraft PAPER spool on the clipboard; wax, staple, label stay", () => {
    const scene = new THREE.Scene();
    const interior = makeInteriorScene();
    scene.add(interior);
    dressWarehouse(scene);
    ...
    const clipboards: THREE.Object3D[] = [];
    dress!.traverse((obj) => {
      if (obj.userData?.kind === "warehouse-spool" && obj.name === "warehouse-spool") {
        spools.push(obj);
      }
      if (obj.userData?.kind === "warehouse-wax" && obj.name === "warehouse-wax") {
        waxes.push(obj);
    ...
    expect(spool.userData.kind).toBe("warehouse-spool");
    expect(spool.userData.mode).toBe("PAPER");
    expect(spool.userData.part).toBe("spool");
    expect(spool.position.y).toBeGreaterThan(1.2);
    expect(spool.position.y).toBeLessThan(2.4);
    const onBack = Math.abs(spool.position.z) > 3.0;
    const onSide = Math.abs(spool.position.x) > 3.5;
    expect(onBack || onSide).toBe(true);
    expect(horizDist(spool, clipboards[0])).toBeLessThan(0.5);
    expect(horizDist(spool, cards[0])).toBeGreaterThan(1.5);
    expect(horizDist(spool, twines[0])).toBeGreaterThan(1.5);
    expect(waxes[0].userData.part).toBe("wax");
    expect(staples[0].userData.part).toBe("staple");
    expect(labels[0].userData.part).toBe("label");
    expect(dockets[0].userData.part).toBe("docket");
    expect(tags[0].userData.part).toBe("tag");
    ...
    spool.traverse((obj) => {
      ...
      if (mesh.isMesh) {
        boxes += 1;
        expect(mesh.geometry.type).toBe("BoxGeometry");
        expect(mesh.userData.kind).toBe("warehouse-spool");
        expect(mesh.userData.mode).toBe("PAPER");
      }
    });
    expect(boxes).toBeGreaterThanOrEqual(1);
...
describe("warehouse PAPER kraft bale", () => {
  it("sits one tiny kraft PAPER bale on the clipboard; spool, wax, staple stay", () => {
    const scene = new THREE.Scene();
    const interior = makeInteriorScene();
    scene.add(interior);
    dressWarehouse(scene);
    ...
    const clipboards: THREE.Object3D[] = [];
    dress!.traverse((obj) => {
      if (obj.userData?.kind === "warehouse-bale" && obj.name === "warehouse-bale") {
        bales.push(obj);
      }
      if (obj.userData?.kind === "warehouse-spool" && obj.name === "warehouse-spool") {
        spools.push(obj);
    ...
    const bale = bales[0];
    expect(bale.userData.kind).toBe("warehouse-bale");
    expect(bale.userData.mode).toBe("PAPER");
    expect(bale.userData.part).toBe("bale");
    expect(bale.position.y).toBeGreaterThan(1.2);
    expect(bale.position.y).toBeLessThan(2.4);
    const onBack = Math.abs(bale.position.z) > 3.0;
    const onSide = Math.abs(bale.position.x) > 3.5;
    expect(onBack || onSide).toBe(true);
    expect(horizDist(bale, clipboards[0])).toBeLessThan(0.5);
    expect(horizDist(bale, cards[0])).toBeGreaterThan(1.5);
    expect(horizDist(bale, twines[0])).toBeGreaterThan(1.5);
    expect(spools[0].userData.part).toBe("spool");
    expect(waxes[0].userData.part).toBe("wax");
    expect(staples[0].userData.part).toBe("staple");
    expect(labels[0].userData.part).toBe("label");
    expect(dockets[0].userData.part).toBe("docket");
    ...
    bale.traverse((obj) => {
      ...
      if (mesh.isMesh) {
        boxes += 1;
        expect(mesh.geometry.type).toBe("BoxGeometry");
        expect(mesh.userData.kind).toBe("warehouse-bale");
        expect(mesh.userData.mode).toBe("PAPER");
      }
    });
    expect(boxes).toBeGreaterThanOrEqual(1);
...
describe("warehouse PAPER kraft cord", () => {
  it("sits one tiny kraft PAPER cord on the clipboard; bale, spool, wax stay", () => {
    const scene = new THREE.Scene();
    const interior = makeInteriorScene();
    scene.add(interior);
    dressWarehouse(scene);
    ...
    const clipboards: THREE.Object3D[] = [];
    dress!.traverse((obj) => {
      if (obj.userData?.kind === "warehouse-cord" && obj.name === "warehouse-cord") {
        cords.push(obj);
      }
      if (obj.userData?.kind === "warehouse-bale" && obj.name === "warehouse-bale") {
        bales.push(obj);
    ...
    const cord = cords[0];
    expect(cord.userData.kind).toBe("warehouse-cord");
    expect(cord.userData.mode).toBe("PAPER");
    expect(cord.userData.part).toBe("cord");
    expect(cord.position.y).toBeGreaterThan(1.2);
    expect(cord.position.y).toBeLessThan(2.4);
    const onBack = Math.abs(cord.position.z) > 3.0;
    const onSide = Math.abs(cord.position.x) > 3.5;
    expect(onBack || onSide).toBe(true);
    expect(horizDist(cord, clipboards[0])).toBeLessThan(0.5);
    expect(horizDist(cord, cards[0])).toBeGreaterThan(1.5);
    expect(horizDist(cord, twines[0])).toBeGreaterThan(1.5);
    expect(bales[0].userData.part).toBe("bale");
    expect(spools[0].userData.part).toBe("spool");
    expect(waxes[0].userData.part).toBe("wax");
    expect(staples[0].userData.part).toBe("staple");
    expect(labels[0].userData.part).toBe("label");
    ...
    cord.traverse((obj) => {
      ...
      if (mesh.isMesh) {
        boxes += 1;
        expect(mesh.geometry.type).toBe("BoxGeometry");
        expect(mesh.userData.kind).toBe("warehouse-cord");
        expect(mesh.userData.mode).toBe("PAPER");
      }
    });
    expect(boxes).toBeGreaterThanOrEqual(1);
