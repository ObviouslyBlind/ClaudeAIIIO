...
describe("taxi roof lamp", () => {
  ...
  it("puts one tiny kraft PAPER headlamp on the taxi; visor, flag, and fare remain", () => {
    const mesh = makeTaxiMesh();
    const headlamps: THREE.Mesh[] = [];
    const visors: THREE.Mesh[] = [];
    const flags: THREE.Mesh[] = [];
    const fares: THREE.Mesh[] = [];
    const kraftHeadlamp = new Set([0xc4a574, 0xf4ead8]);
    mesh.traverse((obj) => {
      const m = obj as THREE.Mesh & { userData: { part?: string; mode?: string } };
    ...
    expect(headlamps.every((h) => h.geometry.type === "BoxGeometry")).toBe(true);
    expect(headlamps.every((h) => h.userData.mode === "PAPER")).toBe(true);
    expect(
      headlamps.every((h) => kraftHeadlamp.has((h.material as THREE.MeshLambertMaterial).color.getHex())),
    ).toBe(true);
    expect(visors.length).toBeGreaterThanOrEqual(1);
    expect(flags.length).toBeGreaterThanOrEqual(1);
    expect(fares.length).toBeGreaterThanOrEqual(1);
  ...
  it("puts one tiny kraft PAPER reflector on the taxi; sidelight, headlamp, and visor remain", () => {
    const mesh = makeTaxiMesh();
    const reflectors: THREE.Mesh[] = [];
    const sidelights: THREE.Mesh[] = [];
    const headlamps: THREE.Mesh[] = [];
    const visors: THREE.Mesh[] = [];
    const kraftReflector = new Set([0xc4a574, 0xf4ead8]);
    mesh.traverse((obj) => {
      const m = obj as THREE.Mesh & { userData: { part?: string; mode?: string } };
    ...
    expect(reflectors.every((r) => r.geometry.type === "BoxGeometry")).toBe(true);
    expect(reflectors.every((r) => r.userData.mode === "PAPER")).toBe(true);
    expect(
      reflectors.every((r) => kraftReflector.has((r.material as THREE.MeshLambertMaterial).color.getHex())),
    ).toBe(true);
    expect(reflectors.every((r) => Math.abs(r.position.x) > 1.0)).toBe(true);
    expect(sidelights.length).toBeGreaterThanOrEqual(1);
    expect(headlamps.length).toBeGreaterThanOrEqual(1);
    expect(visors.length).toBeGreaterThanOrEqual(1);
  });

  it("puts one tiny kraft PAPER grille on the taxi; bumper, headlamp, and tail remain", () => {
    const mesh = makeTaxiMesh();
    ...
    const kraftGrille = new Set([0xc4a574, 0xf4ead8]);
    mesh.traverse((obj) => {
      const m = obj as THREE.Mesh & { userData: { part?: string; mode?: string } };
    ...
    expect(grilles.every((g) => g.geometry.type === "BoxGeometry")).toBe(true);
    expect(grilles.every((g) => g.userData.mode === "PAPER")).toBe(true);
    expect(
      grilles.every((g) => kraftGrille.has((g.material as THREE.MeshLambertMaterial).color.getHex())),
    ).toBe(true);
    expect(grilles.every((g) => g.position.z > 2.0)).toBe(true);
    expect(bumpers.length).toBe(2);
    expect(headlamps.length).toBeGreaterThanOrEqual(1);
    expect(wipers.length).toBeGreaterThanOrEqual(2);
    expect(visors.length).toBeGreaterThanOrEqual(1);
  ...
  it("puts one tiny kraft PAPER tail lamp on the taxi; reflector, sidelight, and headlamp remain", () => {
    ...
    const sidelights: THREE.Mesh[] = [];
    const headlamps: THREE.Mesh[] = [];
    const kraftTail = new Set([0xc4a574, 0xf4ead8]);
    mesh.traverse((obj) => {
      const m = obj as THREE.Mesh & { userData: { part?: string; mode?: string } };
      if (m.userData?.part === "tail") tails.push(m);
      if (m.userData?.part === "reflector") reflectors.push(m);
    ...
    expect(tails.length).toBeGreaterThanOrEqual(1);
    expect(tails.every((t) => t.geometry.type === "BoxGeometry")).toBe(true);
    expect(tails.every((t) => t.userData.mode === "PAPER")).toBe(true);
    expect(
      tails.every((t) => kraftTail.has((t.material as THREE.MeshLambertMaterial).color.getHex())),
    ).toBe(true);
    expect(tails.every((t) => Math.abs(t.position.x) > 1.0)).toBe(true);
    expect(reflectors.length).toBeGreaterThanOrEqual(1);
    expect(sidelights.length).toBeGreaterThanOrEqual(1);
    expect(headlamps.length).toBeGreaterThanOrEqual(1);
  });
