...
describe("hill and verge trees", () => {
  ...
  it("plants PAPER trees on hills, slopes, and behind street lots — not water, quay, tarmac, or dirt", () => {
    ...
    expect(meshes).toBeLessThanOrEqual(29);
  ...
  it("sits one tiny kraft PAPER sap on the north-port palm trunk, acorn and needle remain", () => {
    const map = createLandBoard();
    const scene = { add(_obj: THREE.Object3D) {} };
    const root = makeTrees(map, {
      scene,
    ...
    root.traverse((obj) => {
      const mesh = obj as THREE.Mesh;
      if (!mesh.isMesh) return;
      if (mesh.userData.part === "sap") sapBoxes.push(mesh);
      if (mesh.userData.part === "acorn") acornBoxes.push(mesh);
      if (mesh.userData.part === "needle") needleBoxes.push(mesh);
      if (mesh.userData.part === "lichen") lichenBoxes.push(mesh);
      if (mesh.userData.part === "moss") mossBoxes.push(mesh);
    ...
    const kraft = new Set([0x8a6238, 0x9a6a40, 0x3f7a38, 0x2f6b32]);
    const sap = sapBoxes[0];
    expect(sap.userData.part).toBe("sap");
    expect(sap.userData.dress).toBe("sap");
    expect(sap.userData.mode).toBe("PAPER");
    expect(sap.userData.provenance).toBe("PAPER");
    expect(sap.geometry.type).toBe("BoxGeometry");
    const mat = sap.material as THREE.MeshLambertMaterial;
    expect(mat.type).toBe("MeshLambertMaterial");
    expect(kraft.has(mat.color.getHex())).toBe(true);
    expect(isGrey(mat.color.getHex())).toBe(false);

    sap.geometry.computeBoundingBox();
    const size = new THREE.Vector3();
    sap.geometry.boundingBox!.getSize(size);
    size.multiply(sap.scale);
    expect(size.x).toBeLessThan(0.2);
    expect(size.y).toBeLessThan(0.08);
    expect(size.z).toBeLessThan(0.15);

    const sapPos = new THREE.Vector3();
    sap.getWorldPosition(sapPos);
    const port = ISLANDS.north.port;
    expect(distToPaved(ISLANDS.north, sapPos.x, sapPos.z)).toBeGreaterThanOrEqual(PAVED_CLEAR_M);
    expect(onPublicQuay(ISLANDS.north, sapPos.x, sapPos.z)).toBe(false);
    expect(heightAt(ISLANDS.north, sapPos.x, sapPos.z)).toBeGreaterThanOrEqual(WATER_MIN_M);
    expect(Math.hypot(sapPos.x - port.x, sapPos.z - port.z)).toBeLessThan(400);
    const atTrunk = northPalms.some((p) => Math.hypot(p.x - sapPos.x, p.z - sapPos.z) < 0.5);
    expect(atTrunk).toBe(true);
    const nestPos = new THREE.Vector3();
    nests[0].getWorldPosition(nestPos);
    expect(sapPos.y).toBeGreaterThan(nestPos.y + 0.3);
    ...
    expect(others.length).toBeGreaterThan(0);
    for (const p of others) {
      expect(Math.hypot(sapPos.x - p.x, sapPos.y - p.y, sapPos.z - p.z)).toBeGreaterThan(0.01);
    }
  });

  it("sits one tiny kraft PAPER bud on the north-port palm trunk, sap and acorn remain", () => {
    const map = createLandBoard();
    const scene = { add(_obj: THREE.Object3D) {} };
    const root = makeTrees(map, {
      scene,
    ...
    root.traverse((obj) => {
      const mesh = obj as THREE.Mesh;
      if (!mesh.isMesh) return;
      if (mesh.userData.part === "bud") budBoxes.push(mesh);
      if (mesh.userData.part === "sap") sapBoxes.push(mesh);
      if (mesh.userData.part === "acorn") acornBoxes.push(mesh);
      if (mesh.userData.part === "needle") needleBoxes.push(mesh);
      if (mesh.userData.part === "lichen") lichenBoxes.push(mesh);
    ...
    const kraft = new Set([0x8a6238, 0x9a6a40, 0x3f7a38, 0x2f6b32]);
    const bud = budBoxes[0];
    expect(bud.userData.part).toBe("bud");
    expect(bud.userData.dress).toBe("bud");
    expect(bud.userData.mode).toBe("PAPER");
    expect(bud.userData.provenance).toBe("PAPER");
    expect(bud.geometry.type).toBe("BoxGeometry");
    const mat = bud.material as THREE.MeshLambertMaterial;
    expect(mat.type).toBe("MeshLambertMaterial");
    expect(kraft.has(mat.color.getHex())).toBe(true);
    expect(isGrey(mat.color.getHex())).toBe(false);

    bud.geometry.computeBoundingBox();
    const size = new THREE.Vector3();
    bud.geometry.boundingBox!.getSize(size);
    size.multiply(bud.scale);
    expect(size.x).toBeLessThan(0.2);
    expect(size.y).toBeLessThan(0.08);
    expect(size.z).toBeLessThan(0.15);

    const budPos = new THREE.Vector3();
    bud.getWorldPosition(budPos);
    const port = ISLANDS.north.port;
    expect(distToPaved(ISLANDS.north, budPos.x, budPos.z)).toBeGreaterThanOrEqual(PAVED_CLEAR_M);
    expect(onPublicQuay(ISLANDS.north, budPos.x, budPos.z)).toBe(false);
    expect(heightAt(ISLANDS.north, budPos.x, budPos.z)).toBeGreaterThanOrEqual(WATER_MIN_M);
    expect(Math.hypot(budPos.x - port.x, budPos.z - port.z)).toBeLessThan(400);
    const atTrunk = northPalms.some((p) => Math.hypot(p.x - budPos.x, p.z - budPos.z) < 0.5);
    expect(atTrunk).toBe(true);
    const nestPos = new THREE.Vector3();
    nests[0].getWorldPosition(nestPos);
    expect(budPos.y).toBeGreaterThan(nestPos.y + 0.3);
    ...
    expect(others.length).toBeGreaterThan(0);
    for (const p of others) {
      expect(Math.hypot(budPos.x - p.x, budPos.y - p.y, budPos.z - p.z)).toBeGreaterThan(0.01);
    }
  });

  it("sits one tiny kraft PAPER bloom on the north-port palm trunk, bud and sap remain", () => {
    const map = createLandBoard();
    const scene = { add(_obj: THREE.Object3D) {} };
    const root = makeTrees(map, {
      scene,
    ...
    root.traverse((obj) => {
      const mesh = obj as THREE.Mesh;
      if (!mesh.isMesh) return;
      if (mesh.userData.part === "bloom") bloomBoxes.push(mesh);
      if (mesh.userData.part === "bud") budBoxes.push(mesh);
      if (mesh.userData.part === "sap") sapBoxes.push(mesh);
      if (mesh.userData.part === "acorn") acornBoxes.push(mesh);
      if (mesh.userData.part === "needle") needleBoxes.push(mesh);
    ...
    expect(bloomBoxes.length).toBe(1);
    expect(budBoxes.length).toBe(1);
    expect(sapBoxes.length).toBe(1);
    expect(acornBoxes.length).toBe(1);
    expect(needleBoxes.length).toBe(1);
    ...
    const kraft = new Set([0x8a6238, 0x9a6a40, 0x3f7a38, 0x2f6b32]);
    const bloom = bloomBoxes[0];
    expect(bloom.userData.part).toBe("bloom");
    expect(bloom.userData.dress).toBe("bloom");
    expect(bloom.userData.mode).toBe("PAPER");
    expect(bloom.userData.provenance).toBe("PAPER");
    expect(bloom.geometry.type).toBe("BoxGeometry");
    ...
    const size = new THREE.Vector3();
    bloom.geometry.boundingBox!.getSize(size);
    size.multiply(bloom.scale);
    expect(size.x).toBeLessThan(0.2);
    expect(size.y).toBeLessThan(0.08);
    expect(size.z).toBeLessThan(0.15);

    const bloomPos = new THREE.Vector3();
    bloom.getWorldPosition(bloomPos);
    const port = ISLANDS.north.port;
    expect(distToPaved(ISLANDS.north, bloomPos.x, bloomPos.z)).toBeGreaterThanOrEqual(PAVED_CLEAR_M);
    expect(onPublicQuay(ISLANDS.north, bloomPos.x, bloomPos.z)).toBe(false);
    expect(heightAt(ISLANDS.north, bloomPos.x, bloomPos.z)).toBeGreaterThanOrEqual(WATER_MIN_M);
    expect(Math.hypot(bloomPos.x - port.x, bloomPos.z - port.z)).toBeLessThan(400);
    const atTrunk = northPalms.some((p) => Math.hypot(p.x - bloomPos.x, p.z - bloomPos.z) < 0.5);
    expect(atTrunk).toBe(true);
    const nestPos = new THREE.Vector3();
    nests[0].getWorldPosition(nestPos);
    expect(bloomPos.y).toBeGreaterThan(nestPos.y + 0.3);
    ...
    expect(others.length).toBeGreaterThan(0);
    for (const p of others) {
      expect(Math.hypot(bloomPos.x - p.x, bloomPos.y - p.y, bloomPos.z - p.z)).toBeGreaterThan(0.01);
    }
  });

  it("sits one tiny kraft PAPER catkin on the north-port palm trunk, bloom and bud remain", () => {
    const map = createLandBoard();
    const scene = { add(_obj: THREE.Object3D) {} };
    const root = makeTrees(map, {
      scene,
    ...
    root.traverse((obj) => {
      const mesh = obj as THREE.Mesh;
      if (!mesh.isMesh) return;
      if (mesh.userData.part === "catkin") catkinBoxes.push(mesh);
      if (mesh.userData.part === "bloom") bloomBoxes.push(mesh);
      if (mesh.userData.part === "bud") budBoxes.push(mesh);
      if (mesh.userData.part === "sap") sapBoxes.push(mesh);
      if (mesh.userData.part === "acorn") acornBoxes.push(mesh);
    ...
    expect(catkinBoxes.length).toBe(1);
    expect(bloomBoxes.length).toBe(1);
    expect(budBoxes.length).toBe(1);
    expect(sapBoxes.length).toBe(1);
    expect(acornBoxes.length).toBe(1);
    ...
    const kraft = new Set([0x8a6238, 0x9a6a40, 0x3f7a38, 0x2f6b32]);
    const catkin = catkinBoxes[0];
    expect(catkin.userData.part).toBe("catkin");
    expect(catkin.userData.dress).toBe("catkin");
    expect(catkin.userData.mode).toBe("PAPER");
    expect(catkin.userData.provenance).toBe("PAPER");
    expect(catkin.geometry.type).toBe("BoxGeometry");
    const mat = catkin.material as THREE.MeshLambertMaterial;
    expect(mat.type).toBe("MeshLambertMaterial");
    expect(kraft.has(mat.color.getHex())).toBe(true);
    expect(isGrey(mat.color.getHex())).toBe(false);

    catkin.geometry.computeBoundingBox();
    const size = new THREE.Vector3();
    catkin.geometry.boundingBox!.getSize(size);
    size.multiply(catkin.scale);
    expect(size.x).toBeLessThan(0.2);
    expect(size.y).toBeLessThan(0.08);
    expect(size.z).toBeLessThan(0.15);

    const catkinPos = new THREE.Vector3();
    catkin.getWorldPosition(catkinPos);
    const port = ISLANDS.north.port;
    expect(distToPaved(ISLANDS.north, catkinPos.x, catkinPos.z)).toBeGreaterThanOrEqual(PAVED_CLEAR_M);
    expect(onPublicQuay(ISLANDS.north, catkinPos.x, catkinPos.z)).toBe(false);
    expect(heightAt(ISLANDS.north, catkinPos.x, catkinPos.z)).toBeGreaterThanOrEqual(WATER_MIN_M);
    expect(Math.hypot(catkinPos.x - port.x, catkinPos.z - port.z)).toBeLessThan(400);
    const atTrunk = northPalms.some((p) => Math.hypot(p.x - catkinPos.x, p.z - catkinPos.z) < 0.5);
    expect(atTrunk).toBe(true);
    const nestPos = new THREE.Vector3();
    nests[0].getWorldPosition(nestPos);
    expect(catkinPos.y).toBeGreaterThan(nestPos.y + 0.3);
    ...
    expect(others.length).toBeGreaterThan(0);
    for (const p of others) {
      expect(Math.hypot(catkinPos.x - p.x, catkinPos.y - p.y, catkinPos.z - p.z)).toBeGreaterThan(0.01);
    }
  });

  it("sits one tiny kraft PAPER gall on the north-port palm trunk, catkin and bloom remain", () => {
    const map = createLandBoard();
    const scene = { add(_obj: THREE.Object3D) {} };
    const root = makeTrees(map, {
      scene,
    ...
    root.traverse((obj) => {
      const mesh = obj as THREE.Mesh;
      if (!mesh.isMesh) return;
      if (mesh.userData.part === "gall") gallBoxes.push(mesh);
      if (mesh.userData.part === "catkin") catkinBoxes.push(mesh);
      if (mesh.userData.part === "bloom") bloomBoxes.push(mesh);
      if (mesh.userData.part === "bud") budBoxes.push(mesh);
      if (mesh.userData.part === "sap") sapBoxes.push(mesh);
    ...
    expect(gallBoxes.length).toBe(1);
    expect(catkinBoxes.length).toBe(1);
    expect(bloomBoxes.length).toBe(1);
    expect(budBoxes.length).toBe(1);
    expect(sapBoxes.length).toBe(1);
    ...
    const kraft = new Set([0x8a6238, 0x9a6a40, 0x3f7a38, 0x2f6b32]);
    const gall = gallBoxes[0];
    expect(gall.userData.part).toBe("gall");
    expect(gall.userData.dress).toBe("gall");
    expect(gall.userData.mode).toBe("PAPER");
    expect(gall.userData.provenance).toBe("PAPER");
    expect(gall.geometry.type).toBe("BoxGeometry");
    const mat = gall.material as THREE.MeshLambertMaterial;
    expect(mat.type).toBe("MeshLambertMaterial");
    expect(kraft.has(mat.color.getHex())).toBe(true);
    expect(isGrey(mat.color.getHex())).toBe(false);

    gall.geometry.computeBoundingBox();
    const size = new THREE.Vector3();
    gall.geometry.boundingBox!.getSize(size);
    size.multiply(gall.scale);
    expect(size.x).toBeLessThan(0.2);
    expect(size.y).toBeLessThan(0.08);
    expect(size.z).toBeLessThan(0.15);

    const gallPos = new THREE.Vector3();
    gall.getWorldPosition(gallPos);
    const port = ISLANDS.north.port;
    expect(distToPaved(ISLANDS.north, gallPos.x, gallPos.z)).toBeGreaterThanOrEqual(PAVED_CLEAR_M);
    expect(onPublicQuay(ISLANDS.north, gallPos.x, gallPos.z)).toBe(false);
    expect(heightAt(ISLANDS.north, gallPos.x, gallPos.z)).toBeGreaterThanOrEqual(WATER_MIN_M);
    expect(Math.hypot(gallPos.x - port.x, gallPos.z - port.z)).toBeLessThan(400);
    const atTrunk = northPalms.some((p) => Math.hypot(p.x - gallPos.x, p.z - gallPos.z) < 0.5);
    expect(atTrunk).toBe(true);
    const nestPos = new THREE.Vector3();
    nests[0].getWorldPosition(nestPos);
    expect(gallPos.y).toBeGreaterThan(nestPos.y + 0.3);
    ...
    expect(others.length).toBeGreaterThan(0);
    for (const p of others) {
      expect(Math.hypot(gallPos.x - p.x, gallPos.y - p.y, gallPos.z - p.z)).toBeGreaterThan(0.01);
    }
  });
});
