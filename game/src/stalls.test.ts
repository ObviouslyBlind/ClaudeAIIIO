...
describe("NPC harbour stalls", () => {
  ...
  it("puts one tiny kraft PAPER mango on each NPC stall counter; peach, grape, pear, banana, orange, lime, lemon, plate, napkin, knife, cup, stool remain", () => {
    const mesh = makeStallMesh({ id: "n-test", use: "farm", island: "north", band: "field" });
    const counter = mesh.children.find((c) => c.userData.part === "counter") as THREE.Mesh;
    const mangos = mesh.children.filter((c) => c.userData.part === "mango");
    expect(mangos.length).toBe(1);
    const mango = mangos[0]!;
    expect(mango.userData.mode).toBe("PAPER");
    expect(mango.userData.paper).toBe(true);
    const box = counter.geometry as THREE.BoxGeometry;
    const counterTop = counter.position.y + box.parameters.height / 2;
    expect(mango.position.y).toBeCloseTo(counterTop, 5);
    expect(Math.abs(mango.position.z - counter.position.z)).toBeLessThan(box.parameters.depth / 2);
    expect(Math.abs(mango.position.x)).toBeLessThan(1.7);

    const peach = mesh.children.find((c) => c.userData.part === "peach")!;
    const grape = mesh.children.find((c) => c.userData.part === "grape")!;
    const pear = mesh.children.find((c) => c.userData.part === "pear")!;
    const banana = mesh.children.find((c) => c.userData.part === "banana")!;
    const orange = mesh.children.find((c) => c.userData.part === "orange")!;
    ...
    const melon = mesh.children.find((c) => c.userData.part === "melon")!;
    const cone = mesh.children.find((c) => c.userData.part === "cone")!;
    expect(peach.userData.part).toBe("peach");
    expect(grape.userData.part).toBe("grape");
    expect(pear.userData.part).toBe("pear");
    expect(banana.userData.part).toBe("banana");
    expect(orange.userData.part).toBe("orange");
    ...
    expect(cup.userData.part).toBe("cup");
    expect(stool.userData.part).toBe("stool");
    expect(peach.position.x).toBeCloseTo(-0.22, 5);
    expect(peach.position.y).toBeCloseTo(0.9, 5);
    expect(grape.position.x).toBeCloseTo(0.02, 5);
    expect(grape.position.y).toBeCloseTo(0.9, 5);
    expect(pear.position.x).toBeCloseTo(0.175, 5);
    expect(pear.position.y).toBeCloseTo(0.9, 5);
    ...
    const xz = (a: THREE.Object3D, b: THREE.Object3D) =>
      Math.hypot(a.position.x - b.position.x, a.position.z - b.position.z);
    expect(xz(mango, peach)).toBeGreaterThan(0.15);
    expect(xz(mango, grape)).toBeGreaterThan(0.15);
    expect(xz(mango, pear)).toBeGreaterThan(0.15);
    expect(xz(mango, banana)).toBeGreaterThan(0.15);
    expect(xz(mango, orange)).toBeGreaterThan(0.15);
    expect(xz(mango, lime)).toBeGreaterThan(0.15);
    expect(xz(mango, lemon)).toBeGreaterThan(0.15);
    expect(xz(mango, plate)).toBeGreaterThan(0.15);
    expect(xz(mango, napkin)).toBeGreaterThan(0.15);
    expect(xz(mango, knife)).toBeGreaterThan(0.15);
    expect(xz(mango, cup)).toBeGreaterThan(0.15);
    expect(xz(mango, goods)).toBeGreaterThan(0.15);
    expect(xz(mango, melon)).toBeGreaterThan(0.15);
    expect(xz(mango, cone)).toBeGreaterThan(0.15);
    expect(Math.abs(mango.position.x - stool.position.x)).toBeGreaterThan(0.7);

    const kraft = new Set([0xf4ead8, 0x5a3a22]);
    const colors = hexes(mango);
    expect(colors.length).toBeGreaterThan(0);
    expect(colors.every((c) => kraft.has(c))).toBe(true);
    expect(colors).toContain(0xf4ead8);
    expect(colors).toContain(0x5a3a22);
    ...
    expect(stalls.group.children.length).toBeGreaterThan(0);
    for (const child of stalls.group.children) {
      expect(child.children.filter((c) => c.userData.part === "mango").length).toBe(1);
      expect(child.children.filter((c) => c.userData.part === "peach").length).toBe(1);
      expect(child.children.filter((c) => c.userData.part === "grape").length).toBe(1);
      expect(child.children.filter((c) => c.userData.part === "pear").length).toBe(1);
      expect(child.children.filter((c) => c.userData.part === "banana").length).toBe(1);
  ...
  it("puts one tiny kraft PAPER kiwi on each NPC stall counter; mango, peach, grape, pear, banana, orange, lime, lemon, plate, napkin, knife, cup, stool remain", () => {
    const mesh = makeStallMesh({ id: "n-test", use: "farm", island: "north", band: "field" });
    const counter = mesh.children.find((c) => c.userData.part === "counter") as THREE.Mesh;
    const kiwis = mesh.children.filter((c) => c.userData.part === "kiwi");
    expect(kiwis.length).toBe(1);
    const kiwi = kiwis[0]!;
    expect(kiwi.userData.mode).toBe("PAPER");
    expect(kiwi.userData.paper).toBe(true);
    const box = counter.geometry as THREE.BoxGeometry;
    const counterTop = counter.position.y + box.parameters.height / 2;
    expect(kiwi.position.y).toBeCloseTo(counterTop, 5);
    expect(Math.abs(kiwi.position.z - counter.position.z)).toBeLessThan(box.parameters.depth / 2);
    expect(Math.abs(kiwi.position.x)).toBeLessThan(1.7);

    const mango = mesh.children.find((c) => c.userData.part === "mango")!;
    const peach = mesh.children.find((c) => c.userData.part === "peach")!;
    const grape = mesh.children.find((c) => c.userData.part === "grape")!;
    const pear = mesh.children.find((c) => c.userData.part === "pear")!;
    const banana = mesh.children.find((c) => c.userData.part === "banana")!;
    ...
    const melon = mesh.children.find((c) => c.userData.part === "melon")!;
    const cone = mesh.children.find((c) => c.userData.part === "cone")!;
    expect(mango.userData.part).toBe("mango");
    expect(peach.userData.part).toBe("peach");
    expect(grape.userData.part).toBe("grape");
    expect(pear.userData.part).toBe("pear");
    expect(banana.userData.part).toBe("banana");
    ...
    expect(cup.userData.part).toBe("cup");
    expect(stool.userData.part).toBe("stool");
    expect(mango.position.x).toBeCloseTo(0.5, 5);
    expect(mango.position.y).toBeCloseTo(0.9, 5);
    expect(peach.position.x).toBeCloseTo(-0.22, 5);
    expect(peach.position.y).toBeCloseTo(0.9, 5);
    expect(grape.position.x).toBeCloseTo(0.02, 5);
    expect(grape.position.y).toBeCloseTo(0.9, 5);
    ...
    const xz = (a: THREE.Object3D, b: THREE.Object3D) =>
      Math.hypot(a.position.x - b.position.x, a.position.z - b.position.z);
    expect(xz(kiwi, mango)).toBeGreaterThan(0.15);
    expect(xz(kiwi, peach)).toBeGreaterThan(0.15);
    expect(xz(kiwi, grape)).toBeGreaterThan(0.15);
    expect(xz(kiwi, pear)).toBeGreaterThan(0.15);
    expect(xz(kiwi, banana)).toBeGreaterThan(0.15);
    expect(xz(kiwi, orange)).toBeGreaterThan(0.15);
    expect(xz(kiwi, lime)).toBeGreaterThan(0.15);
    expect(xz(kiwi, lemon)).toBeGreaterThan(0.15);
    expect(xz(kiwi, plate)).toBeGreaterThan(0.15);
    expect(xz(kiwi, napkin)).toBeGreaterThan(0.15);
    expect(xz(kiwi, knife)).toBeGreaterThan(0.15);
    expect(xz(kiwi, cup)).toBeGreaterThan(0.15);
    expect(xz(kiwi, goods)).toBeGreaterThan(0.15);
    expect(xz(kiwi, melon)).toBeGreaterThan(0.15);
    expect(xz(kiwi, cone)).toBeGreaterThan(0.15);
    expect(Math.abs(kiwi.position.x - stool.position.x)).toBeGreaterThan(0.7);

    const kraft = new Set([0xf4ead8, 0x5a3a22]);
    const colors = hexes(kiwi);
    expect(colors.length).toBeGreaterThan(0);
    expect(colors.every((c) => kraft.has(c))).toBe(true);
    expect(colors).toContain(0xf4ead8);
    expect(colors).toContain(0x5a3a22);
    ...
    expect(stalls.group.children.length).toBeGreaterThan(0);
    for (const child of stalls.group.children) {
      expect(child.children.filter((c) => c.userData.part === "kiwi").length).toBe(1);
      expect(child.children.filter((c) => c.userData.part === "mango").length).toBe(1);
      expect(child.children.filter((c) => c.userData.part === "peach").length).toBe(1);
      expect(child.children.filter((c) => c.userData.part === "grape").length).toBe(1);
      expect(child.children.filter((c) => c.userData.part === "pear").length).toBe(1);
  ...
  it("puts one tiny kraft PAPER cherry on each NPC stall counter; kiwi, mango, peach, grape, pear, banana, orange, lime, lemon, plate, napkin, knife, cup, stool remain", () => {
    const mesh = makeStallMesh({ id: "n-test", use: "farm", island: "north", band: "field" });
    const counter = mesh.children.find((c) => c.userData.part === "counter") as THREE.Mesh;
    const cherries = mesh.children.filter((c) => c.userData.part === "cherry");
    expect(cherries.length).toBe(1);
    const cherry = cherries[0]!;
    expect(cherry.userData.mode).toBe("PAPER");
    expect(cherry.userData.paper).toBe(true);
    const box = counter.geometry as THREE.BoxGeometry;
    const counterTop = counter.position.y + box.parameters.height / 2;
    expect(cherry.position.y).toBeCloseTo(counterTop, 5);
    expect(Math.abs(cherry.position.z - counter.position.z)).toBeLessThan(box.parameters.depth / 2);
    expect(Math.abs(cherry.position.x)).toBeLessThan(1.7);

    const kiwi = mesh.children.find((c) => c.userData.part === "kiwi")!;
    const mango = mesh.children.find((c) => c.userData.part === "mango")!;
    const peach = mesh.children.find((c) => c.userData.part === "peach")!;
    const grape = mesh.children.find((c) => c.userData.part === "grape")!;
    const pear = mesh.children.find((c) => c.userData.part === "pear")!;
    ...
    const melon = mesh.children.find((c) => c.userData.part === "melon")!;
    const cone = mesh.children.find((c) => c.userData.part === "cone")!;
    expect(kiwi.userData.part).toBe("kiwi");
    expect(mango.userData.part).toBe("mango");
    expect(peach.userData.part).toBe("peach");
    expect(grape.userData.part).toBe("grape");
    expect(pear.userData.part).toBe("pear");
    ...
    expect(cup.userData.part).toBe("cup");
    expect(stool.userData.part).toBe("stool");
    expect(kiwi.position.x).toBeCloseTo(-0.52, 5);
    expect(kiwi.position.y).toBeCloseTo(0.9, 5);
    expect(mango.position.x).toBeCloseTo(0.5, 5);
    expect(mango.position.y).toBeCloseTo(0.9, 5);
    expect(peach.position.x).toBeCloseTo(-0.22, 5);
    expect(peach.position.y).toBeCloseTo(0.9, 5);
    ...
    const xz = (a: THREE.Object3D, b: THREE.Object3D) =>
      Math.hypot(a.position.x - b.position.x, a.position.z - b.position.z);
    expect(xz(cherry, kiwi)).toBeGreaterThan(0.15);
    expect(xz(cherry, mango)).toBeGreaterThan(0.15);
    expect(xz(cherry, peach)).toBeGreaterThan(0.15);
    expect(xz(cherry, grape)).toBeGreaterThan(0.15);
    expect(xz(cherry, pear)).toBeGreaterThan(0.15);
    expect(xz(cherry, banana)).toBeGreaterThan(0.15);
    expect(xz(cherry, orange)).toBeGreaterThan(0.15);
    expect(xz(cherry, lime)).toBeGreaterThan(0.15);
    expect(xz(cherry, lemon)).toBeGreaterThan(0.15);
    expect(xz(cherry, plate)).toBeGreaterThan(0.15);
    expect(xz(cherry, napkin)).toBeGreaterThan(0.15);
    expect(xz(cherry, knife)).toBeGreaterThan(0.15);
    expect(xz(cherry, cup)).toBeGreaterThan(0.15);
    expect(xz(cherry, goods)).toBeGreaterThan(0.15);
    expect(xz(cherry, melon)).toBeGreaterThan(0.15);
    expect(xz(cherry, cone)).toBeGreaterThan(0.15);
    expect(Math.abs(cherry.position.x - stool.position.x)).toBeGreaterThan(0.7);

    const kraft = new Set([0xf4ead8, 0x5a3a22]);
    const colors = hexes(cherry);
    expect(colors.length).toBeGreaterThan(0);
    expect(colors.every((c) => kraft.has(c))).toBe(true);
    expect(colors).toContain(0xf4ead8);
    expect(colors).toContain(0x5a3a22);
    ...
    expect(stalls.group.children.length).toBeGreaterThan(0);
    for (const child of stalls.group.children) {
      expect(child.children.filter((c) => c.userData.part === "cherry").length).toBe(1);
      expect(child.children.filter((c) => c.userData.part === "kiwi").length).toBe(1);
      expect(child.children.filter((c) => c.userData.part === "mango").length).toBe(1);
      expect(child.children.filter((c) => c.userData.part === "peach").length).toBe(1);
      expect(child.children.filter((c) => c.userData.part === "grape").length).toBe(1);
  ...
  it("puts one tiny kraft PAPER nectarine on each NPC stall counter; cherry, kiwi, mango, peach, grape, pear, banana remain", () => {
    const mesh = makeStallMesh({ id: "n-test", use: "farm", island: "north", band: "field" });
    const counter = mesh.children.find((c) => c.userData.part === "counter") as THREE.Mesh;
    const nectarines = mesh.children.filter((c) => c.userData.part === "nectarine");
    expect(nectarines.length).toBe(1);
    const nectarine = nectarines[0]!;
    expect(nectarine.userData.mode).toBe("PAPER");
    expect(nectarine.userData.paper).toBe(true);
    const box = counter.geometry as THREE.BoxGeometry;
    const counterTop = counter.position.y + box.parameters.height / 2;
    expect(nectarine.position.y).toBeCloseTo(counterTop, 5);
    expect(Math.abs(nectarine.position.z - counter.position.z)).toBeLessThan(box.parameters.depth / 2);
    expect(Math.abs(nectarine.position.x)).toBeLessThan(1.7);

    const cherry = mesh.children.find((c) => c.userData.part === "cherry")!;
    const kiwi = mesh.children.find((c) => c.userData.part === "kiwi")!;
    const mango = mesh.children.find((c) => c.userData.part === "mango")!;
    const peach = mesh.children.find((c) => c.userData.part === "peach")!;
    const grape = mesh.children.find((c) => c.userData.part === "grape")!;
    ...
    const melon = mesh.children.find((c) => c.userData.part === "melon")!;
    const cone = mesh.children.find((c) => c.userData.part === "cone")!;
    expect(cherry.userData.part).toBe("cherry");
    expect(kiwi.userData.part).toBe("kiwi");
    expect(mango.userData.part).toBe("mango");
    expect(peach.userData.part).toBe("peach");
    expect(grape.userData.part).toBe("grape");
    ...
    expect(cup.userData.part).toBe("cup");
    expect(stool.userData.part).toBe("stool");
    expect(cherry.position.x).toBeCloseTo(0.78, 5);
    expect(cherry.position.y).toBeCloseTo(0.9, 5);
    expect(kiwi.position.x).toBeCloseTo(-0.52, 5);
    expect(kiwi.position.y).toBeCloseTo(0.9, 5);
    expect(mango.position.x).toBeCloseTo(0.5, 5);
    expect(mango.position.y).toBeCloseTo(0.9, 5);
    ...
    const xz = (a: THREE.Object3D, b: THREE.Object3D) =>
      Math.hypot(a.position.x - b.position.x, a.position.z - b.position.z);
    expect(xz(nectarine, cherry)).toBeGreaterThan(0.15);
    expect(xz(nectarine, kiwi)).toBeGreaterThan(0.15);
    expect(xz(nectarine, mango)).toBeGreaterThan(0.15);
    expect(xz(nectarine, peach)).toBeGreaterThan(0.15);
    expect(xz(nectarine, grape)).toBeGreaterThan(0.15);
    expect(xz(nectarine, pear)).toBeGreaterThan(0.15);
    expect(xz(nectarine, banana)).toBeGreaterThan(0.15);
    expect(xz(nectarine, orange)).toBeGreaterThan(0.15);
    expect(xz(nectarine, lime)).toBeGreaterThan(0.15);
    expect(xz(nectarine, lemon)).toBeGreaterThan(0.15);
    expect(xz(nectarine, plate)).toBeGreaterThan(0.15);
    expect(xz(nectarine, napkin)).toBeGreaterThan(0.15);
    expect(xz(nectarine, knife)).toBeGreaterThan(0.15);
    expect(xz(nectarine, cup)).toBeGreaterThan(0.15);
    expect(xz(nectarine, goods)).toBeGreaterThan(0.15);
    expect(xz(nectarine, melon)).toBeGreaterThan(0.15);
    expect(xz(nectarine, cone)).toBeGreaterThan(0.15);
    expect(Math.abs(nectarine.position.x - stool.position.x)).toBeGreaterThan(0.7);

    const kraft = new Set([0xf4ead8, 0x5a3a22]);
    const colors = hexes(nectarine);
    expect(colors.length).toBeGreaterThan(0);
    expect(colors.every((c) => kraft.has(c))).toBe(true);
    expect(colors).toContain(0xf4ead8);
    expect(colors).toContain(0x5a3a22);
    ...
    expect(stalls.group.children.length).toBeGreaterThan(0);
    for (const child of stalls.group.children) {
      expect(child.children.filter((c) => c.userData.part === "nectarine").length).toBe(1);
      expect(child.children.filter((c) => c.userData.part === "cherry").length).toBe(1);
      expect(child.children.filter((c) => c.userData.part === "kiwi").length).toBe(1);
      expect(child.children.filter((c) => c.userData.part === "mango").length).toBe(1);
      expect(child.children.filter((c) => c.userData.part === "peach").length).toBe(1);
  ...
  it("puts one tiny kraft PAPER quince on each NPC stall counter; nectarine, cherry, kiwi, mango, peach, grape, pear, banana remain", () => {
    const mesh = makeStallMesh({ id: "n-test", use: "farm", island: "north", band: "field" });
    const counter = mesh.children.find((c) => c.userData.part === "counter") as THREE.Mesh;
    const quinces = mesh.children.filter((c) => c.userData.part === "quince");
    expect(quinces.length).toBe(1);
    const quince = quinces[0]!;
    expect(quince.userData.mode).toBe("PAPER");
    expect(quince.userData.paper).toBe(true);
    const box = counter.geometry as THREE.BoxGeometry;
    const counterTop = counter.position.y + box.parameters.height / 2;
    expect(quince.position.y).toBeCloseTo(counterTop, 5);
    expect(Math.abs(quince.position.z - counter.position.z)).toBeLessThan(box.parameters.depth / 2);
    expect(Math.abs(quince.position.x)).toBeLessThan(1.7);

    const nectarine = mesh.children.find((c) => c.userData.part === "nectarine")!;
    const cherry = mesh.children.find((c) => c.userData.part === "cherry")!;
    const kiwi = mesh.children.find((c) => c.userData.part === "kiwi")!;
    const mango = mesh.children.find((c) => c.userData.part === "mango")!;
    const peach = mesh.children.find((c) => c.userData.part === "peach")!;
    ...
    const melon = mesh.children.find((c) => c.userData.part === "melon")!;
    const cone = mesh.children.find((c) => c.userData.part === "cone")!;
    expect(nectarine.userData.part).toBe("nectarine");
    expect(cherry.userData.part).toBe("cherry");
    expect(kiwi.userData.part).toBe("kiwi");
    expect(mango.userData.part).toBe("mango");
    expect(peach.userData.part).toBe("peach");
    ...
    expect(cup.userData.part).toBe("cup");
    expect(stool.userData.part).toBe("stool");
    expect(nectarine.position.x).toBeCloseTo(-1.05, 5);
    expect(nectarine.position.y).toBeCloseTo(0.9, 5);
    expect(cherry.position.x).toBeCloseTo(0.78, 5);
    expect(cherry.position.y).toBeCloseTo(0.9, 5);
    expect(kiwi.position.x).toBeCloseTo(-0.52, 5);
    expect(kiwi.position.y).toBeCloseTo(0.9, 5);
    ...
    const xz = (a: THREE.Object3D, b: THREE.Object3D) =>
      Math.hypot(a.position.x - b.position.x, a.position.z - b.position.z);
    expect(xz(quince, nectarine)).toBeGreaterThan(0.15);
    expect(xz(quince, cherry)).toBeGreaterThan(0.15);
    expect(xz(quince, kiwi)).toBeGreaterThan(0.15);
    expect(xz(quince, mango)).toBeGreaterThan(0.15);
    expect(xz(quince, peach)).toBeGreaterThan(0.15);
    expect(xz(quince, grape)).toBeGreaterThan(0.15);
    expect(xz(quince, pear)).toBeGreaterThan(0.15);
    expect(xz(quince, banana)).toBeGreaterThan(0.15);
    expect(xz(quince, orange)).toBeGreaterThan(0.15);
    expect(xz(quince, lime)).toBeGreaterThan(0.15);
    expect(xz(quince, lemon)).toBeGreaterThan(0.15);
    expect(xz(quince, plate)).toBeGreaterThan(0.15);
    expect(xz(quince, napkin)).toBeGreaterThan(0.15);
    expect(xz(quince, knife)).toBeGreaterThan(0.15);
    expect(xz(quince, cup)).toBeGreaterThan(0.15);
    expect(xz(quince, goods)).toBeGreaterThan(0.15);
    expect(xz(quince, melon)).toBeGreaterThan(0.15);
    expect(xz(quince, cone)).toBeGreaterThan(0.15);
    expect(Math.abs(quince.position.x - stool.position.x)).toBeGreaterThan(0.7);

    const kraft = new Set([0xf4ead8, 0x5a3a22]);
    const colors = hexes(quince);
    expect(colors.length).toBeGreaterThan(0);
    expect(colors.every((c) => kraft.has(c))).toBe(true);
    expect(colors).toContain(0xf4ead8);
    expect(colors).toContain(0x5a3a22);
    ...
    expect(stalls.group.children.length).toBeGreaterThan(0);
    for (const child of stalls.group.children) {
      expect(child.children.filter((c) => c.userData.part === "quince").length).toBe(1);
      expect(child.children.filter((c) => c.userData.part === "nectarine").length).toBe(1);
      expect(child.children.filter((c) => c.userData.part === "cherry").length).toBe(1);
      expect(child.children.filter((c) => c.userData.part === "kiwi").length).toBe(1);
      expect(child.children.filter((c) => c.userData.part === "mango").length).toBe(1);
  ...
  it("puts one tiny kraft PAPER papaya on each NPC stall counter; quince, nectarine, cherry, kiwi, mango, peach, grape, pear, banana remain", () => {
    const mesh = makeStallMesh({ id: "n-test", use: "farm", island: "north", band: "field" });
    const counter = mesh.children.find((c) => c.userData.part === "counter") as THREE.Mesh;
    const papayas = mesh.children.filter((c) => c.userData.part === "papaya");
    expect(papayas.length).toBe(1);
    const papaya = papayas[0]!;
    expect(papaya.userData.mode).toBe("PAPER");
    expect(papaya.userData.paper).toBe(true);
    const box = counter.geometry as THREE.BoxGeometry;
    const counterTop = counter.position.y + box.parameters.height / 2;
    expect(papaya.position.y).toBeCloseTo(counterTop, 5);
    expect(Math.abs(papaya.position.z - counter.position.z)).toBeLessThan(box.parameters.depth / 2);
    expect(Math.abs(papaya.position.x)).toBeLessThan(1.7);

    const quince = mesh.children.find((c) => c.userData.part === "quince")!;
    const nectarine = mesh.children.find((c) => c.userData.part === "nectarine")!;
    const cherry = mesh.children.find((c) => c.userData.part === "cherry")!;
    const kiwi = mesh.children.find((c) => c.userData.part === "kiwi")!;
    const mango = mesh.children.find((c) => c.userData.part === "mango")!;
    ...
    const melon = mesh.children.find((c) => c.userData.part === "melon")!;
    const cone = mesh.children.find((c) => c.userData.part === "cone")!;
    expect(quince.userData.part).toBe("quince");
    expect(nectarine.userData.part).toBe("nectarine");
    expect(cherry.userData.part).toBe("cherry");
    expect(kiwi.userData.part).toBe("kiwi");
    expect(mango.userData.part).toBe("mango");
    ...
    expect(cup.userData.part).toBe("cup");
    expect(stool.userData.part).toBe("stool");
    expect(quince.position.x).toBeCloseTo(1.1, 5);
    expect(quince.position.y).toBeCloseTo(0.9, 5);
    expect(nectarine.position.x).toBeCloseTo(-1.05, 5);
    expect(nectarine.position.y).toBeCloseTo(0.9, 5);
    expect(cherry.position.x).toBeCloseTo(0.78, 5);
    expect(cherry.position.y).toBeCloseTo(0.9, 5);
    ...
    const xz = (a: THREE.Object3D, b: THREE.Object3D) =>
      Math.hypot(a.position.x - b.position.x, a.position.z - b.position.z);
    expect(xz(papaya, quince)).toBeGreaterThan(0.15);
    expect(xz(papaya, nectarine)).toBeGreaterThan(0.15);
    expect(xz(papaya, cherry)).toBeGreaterThan(0.15);
    expect(xz(papaya, kiwi)).toBeGreaterThan(0.15);
    expect(xz(papaya, mango)).toBeGreaterThan(0.15);
    expect(xz(papaya, peach)).toBeGreaterThan(0.15);
    expect(xz(papaya, grape)).toBeGreaterThan(0.15);
    expect(xz(papaya, pear)).toBeGreaterThan(0.15);
    expect(xz(papaya, banana)).toBeGreaterThan(0.15);
    expect(xz(papaya, orange)).toBeGreaterThan(0.15);
    expect(xz(papaya, lime)).toBeGreaterThan(0.15);
    expect(xz(papaya, lemon)).toBeGreaterThan(0.15);
    expect(xz(papaya, plate)).toBeGreaterThan(0.15);
    expect(xz(papaya, napkin)).toBeGreaterThan(0.15);
    expect(xz(papaya, knife)).toBeGreaterThan(0.15);
    expect(xz(papaya, cup)).toBeGreaterThan(0.15);
    expect(xz(papaya, goods)).toBeGreaterThan(0.15);
    expect(xz(papaya, melon)).toBeGreaterThan(0.15);
    expect(xz(papaya, cone)).toBeGreaterThan(0.15);
    expect(Math.abs(papaya.position.x - stool.position.x)).toBeGreaterThan(0.7);

    const kraft = new Set([0xf4ead8, 0x5a3a22]);
    const colors = hexes(papaya);
    expect(colors.length).toBeGreaterThan(0);
    expect(colors.every((c) => kraft.has(c))).toBe(true);
    expect(colors).toContain(0xf4ead8);
    expect(colors).toContain(0x5a3a22);
    ...
    expect(stalls.group.children.length).toBeGreaterThan(0);
    for (const child of stalls.group.children) {
      expect(child.children.filter((c) => c.userData.part === "papaya").length).toBe(1);
      expect(child.children.filter((c) => c.userData.part === "quince").length).toBe(1);
      expect(child.children.filter((c) => c.userData.part === "nectarine").length).toBe(1);
      expect(child.children.filter((c) => c.userData.part === "cherry").length).toBe(1);
      expect(child.children.filter((c) => c.userData.part === "kiwi").length).toBe(1);
  ...
  it("puts one tiny kraft PAPER guava on each NPC stall counter; papaya, quince, nectarine, cherry, kiwi, mango, peach, grape, pear, banana remain", () => {
    const mesh = makeStallMesh({ id: "n-test", use: "farm", island: "north", band: "field" });
    const counter = mesh.children.find((c) => c.userData.part === "counter") as THREE.Mesh;
    const guavas = mesh.children.filter((c) => c.userData.part === "guava");
    expect(guavas.length).toBe(1);
    const guava = guavas[0]!;
    expect(guava.userData.mode).toBe("PAPER");
    expect(guava.userData.paper).toBe(true);
    const box = counter.geometry as THREE.BoxGeometry;
    const counterTop = counter.position.y + box.parameters.height / 2;
    expect(guava.position.y).toBeCloseTo(counterTop, 5);
    expect(Math.abs(guava.position.z - counter.position.z)).toBeLessThan(box.parameters.depth / 2);
    expect(Math.abs(guava.position.x)).toBeLessThan(1.7);

    const papaya = mesh.children.find((c) => c.userData.part === "papaya")!;
    const quince = mesh.children.find((c) => c.userData.part === "quince")!;
    const nectarine = mesh.children.find((c) => c.userData.part === "nectarine")!;
    const cherry = mesh.children.find((c) => c.userData.part === "cherry")!;
    const kiwi = mesh.children.find((c) => c.userData.part === "kiwi")!;
    ...
    const melon = mesh.children.find((c) => c.userData.part === "melon")!;
    const cone = mesh.children.find((c) => c.userData.part === "cone")!;
    expect(papaya.userData.part).toBe("papaya");
    expect(quince.userData.part).toBe("quince");
    expect(nectarine.userData.part).toBe("nectarine");
    expect(cherry.userData.part).toBe("cherry");
    expect(kiwi.userData.part).toBe("kiwi");
    ...
    expect(cup.userData.part).toBe("cup");
    expect(stool.userData.part).toBe("stool");
    expect(papaya.position.x).toBeCloseTo(-1.35, 5);
    expect(papaya.position.y).toBeCloseTo(0.9, 5);
    expect(quince.position.x).toBeCloseTo(1.1, 5);
    expect(quince.position.y).toBeCloseTo(0.9, 5);
    expect(nectarine.position.x).toBeCloseTo(-1.05, 5);
    expect(nectarine.position.y).toBeCloseTo(0.9, 5);
    ...
    const xz = (a: THREE.Object3D, b: THREE.Object3D) =>
      Math.hypot(a.position.x - b.position.x, a.position.z - b.position.z);
    expect(xz(guava, papaya)).toBeGreaterThan(0.15);
    expect(xz(guava, quince)).toBeGreaterThan(0.15);
    expect(xz(guava, nectarine)).toBeGreaterThan(0.15);
    expect(xz(guava, cherry)).toBeGreaterThan(0.15);
    expect(xz(guava, kiwi)).toBeGreaterThan(0.15);
    expect(xz(guava, mango)).toBeGreaterThan(0.15);
    expect(xz(guava, peach)).toBeGreaterThan(0.15);
    expect(xz(guava, grape)).toBeGreaterThan(0.15);
    expect(xz(guava, pear)).toBeGreaterThan(0.15);
    expect(xz(guava, banana)).toBeGreaterThan(0.15);
    expect(xz(guava, orange)).toBeGreaterThan(0.15);
    expect(xz(guava, lime)).toBeGreaterThan(0.15);
    expect(xz(guava, lemon)).toBeGreaterThan(0.15);
    expect(xz(guava, plate)).toBeGreaterThan(0.15);
    expect(xz(guava, napkin)).toBeGreaterThan(0.15);
    expect(xz(guava, knife)).toBeGreaterThan(0.15);
    expect(xz(guava, cup)).toBeGreaterThan(0.15);
    expect(xz(guava, goods)).toBeGreaterThan(0.15);
    expect(xz(guava, melon)).toBeGreaterThan(0.15);
    expect(xz(guava, cone)).toBeGreaterThan(0.15);
    expect(Math.abs(guava.position.x - stool.position.x)).toBeGreaterThan(0.7);

    const kraft = new Set([0xf4ead8, 0x5a3a22]);
    const colors = hexes(guava);
    expect(colors.length).toBeGreaterThan(0);
    expect(colors.every((c) => kraft.has(c))).toBe(true);
    expect(colors).toContain(0xf4ead8);
    expect(colors).toContain(0x5a3a22);
    ...
    expect(stalls.group.children.length).toBeGreaterThan(0);
    for (const child of stalls.group.children) {
      expect(child.children.filter((c) => c.userData.part === "guava").length).toBe(1);
      expect(child.children.filter((c) => c.userData.part === "papaya").length).toBe(1);
      expect(child.children.filter((c) => c.userData.part === "quince").length).toBe(1);
      expect(child.children.filter((c) => c.userData.part === "nectarine").length).toBe(1);
      expect(child.children.filter((c) => c.userData.part === "cherry").length).toBe(1);
  ...
  it("puts one tiny kraft PAPER currant on each NPC stall counter; guava, papaya, quince, nectarine, cherry, kiwi, mango, peach, grape, pear, banana remain", () => {
    const mesh = makeStallMesh({ id: "n-test", use: "farm", island: "north", band: "field" });
    const counter = mesh.children.find((c) => c.userData.part === "counter") as THREE.Mesh;
    const currants = mesh.children.filter((c) => c.userData.part === "currant");
    expect(currants.length).toBe(1);
    const currant = currants[0]!;
    expect(currant.userData.mode).toBe("PAPER");
    expect(currant.userData.paper).toBe(true);
    const box = counter.geometry as THREE.BoxGeometry;
    const counterTop = counter.position.y + box.parameters.height / 2;
    expect(currant.position.y).toBeCloseTo(counterTop, 5);
    expect(Math.abs(currant.position.z - counter.position.z)).toBeLessThan(box.parameters.depth / 2);
    expect(Math.abs(currant.position.x)).toBeLessThan(1.7);

    const guava = mesh.children.find((c) => c.userData.part === "guava")!;
    const papaya = mesh.children.find((c) => c.userData.part === "papaya")!;
    const quince = mesh.children.find((c) => c.userData.part === "quince")!;
    const nectarine = mesh.children.find((c) => c.userData.part === "nectarine")!;
    const cherry = mesh.children.find((c) => c.userData.part === "cherry")!;
    ...
    const melon = mesh.children.find((c) => c.userData.part === "melon")!;
    const cone = mesh.children.find((c) => c.userData.part === "cone")!;
    expect(guava.userData.part).toBe("guava");
    expect(papaya.userData.part).toBe("papaya");
    expect(quince.userData.part).toBe("quince");
    expect(nectarine.userData.part).toBe("nectarine");
    expect(cherry.userData.part).toBe("cherry");
    ...
    expect(cup.userData.part).toBe("cup");
    expect(stool.userData.part).toBe("stool");
    expect(guava.position.x).toBeCloseTo(-0.78, 5);
    expect(guava.position.y).toBeCloseTo(0.9, 5);
    expect(papaya.position.x).toBeCloseTo(-1.35, 5);
    expect(papaya.position.y).toBeCloseTo(0.9, 5);
    expect(quince.position.x).toBeCloseTo(1.1, 5);
    expect(quince.position.y).toBeCloseTo(0.9, 5);
    ...
    const xz = (a: THREE.Object3D, b: THREE.Object3D) =>
      Math.hypot(a.position.x - b.position.x, a.position.z - b.position.z);
    expect(xz(currant, guava)).toBeGreaterThan(0.15);
    expect(xz(currant, papaya)).toBeGreaterThan(0.15);
    expect(xz(currant, quince)).toBeGreaterThan(0.15);
    expect(xz(currant, nectarine)).toBeGreaterThan(0.15);
    expect(xz(currant, cherry)).toBeGreaterThan(0.15);
    expect(xz(currant, kiwi)).toBeGreaterThan(0.15);
    expect(xz(currant, mango)).toBeGreaterThan(0.15);
    expect(xz(currant, peach)).toBeGreaterThan(0.15);
    expect(xz(currant, grape)).toBeGreaterThan(0.15);
    expect(xz(currant, pear)).toBeGreaterThan(0.15);
    expect(xz(currant, banana)).toBeGreaterThan(0.15);
    expect(xz(currant, orange)).toBeGreaterThan(0.15);
    expect(xz(currant, lime)).toBeGreaterThan(0.15);
    expect(xz(currant, lemon)).toBeGreaterThan(0.15);
    expect(xz(currant, plate)).toBeGreaterThan(0.15);
    expect(xz(currant, napkin)).toBeGreaterThan(0.15);
    expect(xz(currant, knife)).toBeGreaterThan(0.15);
    expect(xz(currant, cup)).toBeGreaterThan(0.15);
    expect(xz(currant, goods)).toBeGreaterThan(0.15);
    expect(xz(currant, melon)).toBeGreaterThan(0.15);
    expect(xz(currant, cone)).toBeGreaterThan(0.15);
    expect(Math.abs(currant.position.x - stool.position.x)).toBeGreaterThan(0.7);

    const kraft = new Set([0xf4ead8, 0x5a3a22]);
    const colors = hexes(currant);
    expect(colors.length).toBeGreaterThan(0);
    expect(colors.every((c) => kraft.has(c))).toBe(true);
    expect(colors).toContain(0xf4ead8);
    expect(colors).toContain(0x5a3a22);
    ...
    expect(stalls.group.children.length).toBeGreaterThan(0);
    for (const child of stalls.group.children) {
      expect(child.children.filter((c) => c.userData.part === "currant").length).toBe(1);
      expect(child.children.filter((c) => c.userData.part === "guava").length).toBe(1);
      expect(child.children.filter((c) => c.userData.part === "papaya").length).toBe(1);
      expect(child.children.filter((c) => c.userData.part === "quince").length).toBe(1);
      expect(child.children.filter((c) => c.userData.part === "nectarine").length).toBe(1);
