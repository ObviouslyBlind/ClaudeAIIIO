...
describe("player PAPER handcart", () => {
  ...
  it("builds a crate bed, two wheels, and handles in harbour crate wood", () => {
    ...
    expect(CART_MESH_COUNT).toBe(46);
  ...
  it("puts one tiny kraft PAPER hazel on the cart bed; walnut, olive, date, apricot, fig, plum, yam, parsnip, squash, radish, beet, turnip, leek, cabbage, garlic, onion, potato, carrot, apple remain", () => {
    const player = makePlayer();
    dressCart(player);
    const cart = player.getObjectByName("paper-cart")!;
    const p = parts(cart);
    expect(p.filter((k) => k === "hazel").length).toBe(1);
    expect(p.filter((k) => k === "walnut").length).toBeGreaterThanOrEqual(1);
    expect(p.filter((k) => k === "olive").length).toBeGreaterThanOrEqual(1);
    expect(p.filter((k) => k === "date").length).toBeGreaterThanOrEqual(1);
    expect(p.filter((k) => k === "apricot").length).toBeGreaterThanOrEqual(1);
    expect(p.filter((k) => k === "fig").length).toBeGreaterThanOrEqual(1);
    ...
    const hazel = cart.children.find((c) => c.userData.part === "hazel") as THREE.Mesh;
    const walnut = cart.children.find((c) => c.userData.part === "walnut") as THREE.Mesh;
    const olive = cart.children.find((c) => c.userData.part === "olive") as THREE.Mesh;
    const date = cart.children.find((c) => c.userData.part === "date") as THREE.Mesh;
    const apricot = cart.children.find((c) => c.userData.part === "apricot") as THREE.Mesh;
    ...
    const apple = cart.children.find((c) => c.userData.part === "apple") as THREE.Mesh;
    const bed = cart.children.find((c) => c.userData.part === "bed")!;
    expect(hazel.userData.mode).toBe("PAPER");
    expect(hazel.geometry.type).toBe("BoxGeometry");
    expect(hazel.position.y).toBeGreaterThan(bed.position.y);
    expect(Math.hypot(hazel.position.x - walnut.position.x, hazel.position.z - walnut.position.z)).toBeGreaterThan(0.12);
    expect(Math.hypot(hazel.position.x - olive.position.x, hazel.position.z - olive.position.z)).toBeGreaterThan(0.12);
    expect(Math.hypot(hazel.position.x - date.position.x, hazel.position.z - date.position.z)).toBeGreaterThan(0.12);
    expect(Math.hypot(hazel.position.x - apricot.position.x, hazel.position.z - apricot.position.z)).toBeGreaterThan(0.12);
    expect(Math.hypot(hazel.position.x - fig.position.x, hazel.position.z - fig.position.z)).toBeGreaterThan(0.12);
    expect(Math.hypot(hazel.position.x - plum.position.x, hazel.position.z - plum.position.z)).toBeGreaterThan(0.12);
    expect(Math.hypot(hazel.position.x - yam.position.x, hazel.position.z - yam.position.z)).toBeGreaterThan(0.12);
    expect(Math.hypot(hazel.position.x - parsnip.position.x, hazel.position.z - parsnip.position.z)).toBeGreaterThan(0.12);
    expect(Math.hypot(hazel.position.x - squash.position.x, hazel.position.z - squash.position.z)).toBeGreaterThan(0.12);
    expect(Math.hypot(hazel.position.x - radish.position.x, hazel.position.z - radish.position.z)).toBeGreaterThan(0.12);
    expect(Math.hypot(hazel.position.x - beet.position.x, hazel.position.z - beet.position.z)).toBeGreaterThan(0.12);
    expect(Math.hypot(hazel.position.x - turnip.position.x, hazel.position.z - turnip.position.z)).toBeGreaterThan(0.12);
    expect(Math.hypot(hazel.position.x - leek.position.x, hazel.position.z - leek.position.z)).toBeGreaterThan(0.12);
    expect(Math.hypot(hazel.position.x - cabbage.position.x, hazel.position.z - cabbage.position.z)).toBeGreaterThan(0.12);
    expect(Math.hypot(hazel.position.x - garlic.position.x, hazel.position.z - garlic.position.z)).toBeGreaterThan(0.12);
    expect(Math.hypot(hazel.position.x - onion.position.x, hazel.position.z - onion.position.z)).toBeGreaterThan(0.12);
    expect(Math.hypot(hazel.position.x - potato.position.x, hazel.position.z - potato.position.z)).toBeGreaterThan(0.12);
    expect(Math.hypot(hazel.position.x - carrot.position.x, hazel.position.z - carrot.position.z)).toBeGreaterThan(0.12);
    expect(Math.hypot(hazel.position.x - apple.position.x, hazel.position.z - apple.position.z)).toBeGreaterThan(0.12);
    const hex = (hazel.material as THREE.MeshLambertMaterial).color.getHex();
    expect([0x8a6238, 0x9a6a40]).toContain(hex);
    expect(isGrey(hex)).toBe(false);
    const { width, height, depth } = (hazel.geometry as THREE.BoxGeometry).parameters;
    expect(width).toBeLessThan(0.12);
    expect(height).toBeLessThan(0.12);
    expect(depth).toBeLessThan(0.12);
  });
