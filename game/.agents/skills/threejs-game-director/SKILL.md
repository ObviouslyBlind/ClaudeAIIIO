...
description: "Three.js arcade/from-scratch director. In this repo, load two-harbors-director FIRST. Do not scaffold a new Vite game, add WASD, or replace game/src. Use this pack for harbour visual polish, glTF, lighting, and QA evidence only after Two Harbors rules."
---

# Three.js Game Director

> **Two Harbors:** This is a persistent harbour shard, not an empty arcade repo.
> Read `.cursor/skills/two-harbors-director/SKILL.md` first. Do **not** run
> `create_threejs_game.py`. Do **not** add WASD, pointer lock, or OrbitControls
> in play. Sim stays in `game/src`. Client stays in `game/public/harbour`.
> Politics frozen. PAPER / SIMULATED. Phone 30fps Low, no post-process.
