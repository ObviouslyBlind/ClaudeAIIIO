...
description: "Three.js gameplay systems (scaffold, combat, juice). In Two Harbors, do not create a new Vite game or add combat/WASD. Use only for camera/feel notes after two-harbors-controls and two-harbors-camera."
---

# Three.js Gameplay Systems

> **Two Harbors:** Do not scaffold. Do not add combat, score, waves, or WASD.
> Playable loop is already spawn → lease → cart → taxi → ferry. See
> `two-harbors-director` and `two-harbors-island-play`.
