# 2Isles version

PAPER / SIMULATED. No wallet.

**Now: Alpha 0.5.1** — the **buildings push**, playable. Spawn **$10,000**. Split Lots / Properties / Landlord. Confirm buy, enter that room, Place from inventory, tenant profiles, South walkers. Dollhouse camera (RMB-hold orbit). Not Blender.

Always update this file and [HANDOVER.md](HANDOVER.md) in the same change.

## Scheme

| Lane | Number |
|---|---|
| Alpha | `0.x` and `0.x.y` |
| **0.5.1** | The whole buildings / rooms push. Playable: confirm buy, enter that room, Place from inventory, tenant profiles, South walkers. Camera and sheets stay **0.5.1**. Blender meshes are not a version bump. |
| Next number | Only after 0.5.1 is done. That next number is a **different** feature, not the harbour camera. |
| Beta | **1.0** |

Do not bump because a file landed. Bump when the named push is playable.

Read the next feature out in [HANDOVER.md](HANDOVER.md) **before** writing code for it.

## History

| Version | Date | What |
|---|---|---|
| 0.5 | 2026-08-20 | Start of the buildings push. Spec accepted. |
| 0.5.1 | 2026-08-21 | **This push, playable.** Spawn **$10,000**. Split menus, confirm buy, enter room, Place from inventory, tenant profiles, South walkers. Dirt $15,000. Blender shells later. |
