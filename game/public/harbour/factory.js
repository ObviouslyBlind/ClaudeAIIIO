...
/**
 * Tiny kraft PAPER auger: KRAFT handle, KRAFT_LIGHT bit. Boxes only.
 * Sits on the wood bench — not the adze, clamp, vice, plane, gouge, mallet, chisel, file, rasp, awl, cork, funnel, oilcan, peg, shaving, rag, rivet, or wrench.
 */
function paperAuger(x, z, yaw = 0) {
  const g = new THREE.Group();
  g.name = "factory-paper-auger";
  g.userData.kind = "factory-paper-auger";
  g.userData.mode = "PAPER";
  g.userData.part = "auger";
  // Wood-bench top: y0 0.16 + 0.78, top half 0.04.
  const sitY = 0.16 + 0.78 + 0.04;
  g.position.set(x, sitY, z);
  g.rotation.y = yaw;
  const handle = paperBox(0.042, 0.016, 0.016, KRAFT, "factory-paper-auger");
  handle.position.set(-0.012, 0.01, 0);
  const bit = paperBox(0.038, 0.01, 0.01, KRAFT_LIGHT, "factory-paper-auger");
  bit.position.set(0.02, 0.01, 0);
  g.add(handle, bit);
  return g;
}
...
/**
 * Tiny kraft PAPER spoke: KRAFT shaft, KRAFT_LIGHT hub. Boxes only.
 * Sits on the wood bench — not the drawknife, auger, adze, clamp, vice, plane, gouge, mallet, chisel, file, rasp, awl, cork, funnel, oilcan, peg, shaving, rag, or rivet.
 */
function paperSpoke(x, z, yaw = 0) {
  const g = new THREE.Group();
  g.name = "factory-paper-spoke";
  g.userData.kind = "factory-paper-spoke";
  g.userData.mode = "PAPER";
  g.userData.part = "spoke";
  // Wood-bench top: y0 0.16 + 0.78, top half 0.04.
  const sitY = 0.16 + 0.78 + 0.04;
  g.position.set(x, sitY, z);
  g.rotation.y = yaw;
  const shaft = paperBox(0.048, 0.01, 0.01, KRAFT, "factory-paper-spoke");
  shaft.position.set(-0.006, 0.008, 0);
  const hub = paperBox(0.012, 0.016, 0.016, KRAFT_LIGHT, "factory-paper-spoke");
  hub.position.set(0.022, 0.01, 0);
  g.add(shaft, hub);
  return g;
}

/**
 * Tiny kraft PAPER bit: KRAFT shank, KRAFT_LIGHT tip. Boxes only.
 * Sits on the wood bench — not the spoke, drawknife, auger, adze, clamp, vice, plane, gouge, mallet, chisel, file, rasp, awl, cork, funnel, oilcan, peg, shaving, rag, or rivet.
 */
function paperBit(x, z, yaw = 0) {
  const g = new THREE.Group();
  g.name = "factory-paper-bit";
  g.userData.kind = "factory-paper-bit";
  g.userData.mode = "PAPER";
  g.userData.part = "bit";
  // Wood-bench top: y0 0.16 + 0.78, top half 0.04.
  const sitY = 0.16 + 0.78 + 0.04;
  g.position.set(x, sitY, z);
  g.rotation.y = yaw;
  const shank = paperBox(0.036, 0.012, 0.012, KRAFT, "factory-paper-bit");
  shank.position.set(-0.008, 0.008, 0);
  const tip = paperBox(0.028, 0.008, 0.008, KRAFT_LIGHT, "factory-paper-bit");
  tip.position.set(0.022, 0.008, 0);
  g.add(shank, tip);
  return g;
}
...
function makeFactoryDress() {
  ...
  // Wood-bench east-south, south of chisel. Off drawknife, auger, adze, clamp, vice, plane, gouge, mallet, chisel, file, rasp, awl, cork, funnel, oilcan, peg, shaving, rag, rivet.
  g.add(paperSpoke(3.42, -1.82, 0.05));
  // Wood-bench south-west, south of file. Off spoke, drawknife, auger, adze, clamp, vice, plane, gouge, mallet, chisel, file, rasp, awl, cork, funnel, oilcan, peg, shaving, rag, rivet.
  g.add(paperBit(2.96, -2.12, 0.05));
