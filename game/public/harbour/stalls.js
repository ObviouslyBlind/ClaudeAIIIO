...
export function makeStallMesh(plot) {
  ...
  // One tiny kraft PAPER banana on the counter so the stall reads as a
  // working stand, not only produce. Corn body + wood stem. Local
  // offset only — stall world pose stays put. PAPER boxes. Existing
  // hexes. Offset from orange, lime, lemon, plate, napkin, knife, cup, stool.
  if (!g.children.some((c) => c.userData.part === "banana")) {
    const banana = new THREE.Group();
    banana.name = "banana";
    banana.userData.part = "banana";
    banana.userData.mode = "PAPER";
    banana.userData.paper = true;
    banana.position.set(-1.52, 0.9, 1.05);
    const body = paperBox(0.11, 0.05, 0.05, CORN, false);
    body.position.y = 0.025;
    body.userData.part = "banana";
    const stem = paperBox(0.02, 0.025, 0.02, WOOD, false);
    stem.position.set(-0.055, 0.05, 0);
    stem.userData.part = "banana";
    banana.add(body, stem);
    g.add(banana);
  }

  // One tiny kraft PAPER pear on the counter so the stall reads as a
  // working stand, not only produce. Melon body + wood stem. Local
  // offset only — stall world pose stays put. PAPER boxes. Existing
  // hexes. Offset from banana, orange, lime, lemon, plate, napkin, knife, cup, stool.
  if (!g.children.some((c) => c.userData.part === "pear")) {
    const pear = new THREE.Group();
    pear.name = "pear";
    pear.userData.part = "pear";
    pear.userData.mode = "PAPER";
    pear.userData.paper = true;
    pear.position.set(0.175, 0.9, 1.05);
    const body = paperBox(0.07, 0.09, 0.07, MELON, false);
    body.position.y = 0.045;
    body.userData.part = "pear";
    const stem = paperBox(0.02, 0.025, 0.02, WOOD, false);
    stem.position.y = 0.105;
    stem.userData.part = "pear";
    pear.add(body, stem);
    g.add(pear);
  }

  // One tiny kraft PAPER grape on the counter so the stall reads as a
  // working stand, not only produce. Kraft body + wood stem. Local
  // offset only — stall world pose stays put. PAPER boxes. Existing
  // hexes. Offset from pear, banana, orange, lime, lemon, plate, napkin, knife, cup.
  if (!g.children.some((c) => c.userData.part === "grape")) {
    const grape = new THREE.Group();
    grape.name = "grape";
    grape.userData.part = "grape";
    grape.userData.mode = "PAPER";
    grape.userData.paper = true;
    grape.position.set(0.02, 0.9, 1.18);
    const body = paperBox(0.05, 0.05, 0.05, KRAFT, false);
    body.position.y = 0.025;
    body.userData.part = "grape";
    const stem = paperBox(0.015, 0.02, 0.015, WOOD, false);
    stem.position.y = 0.06;
    stem.userData.part = "grape";
    grape.add(body, stem);
    g.add(grape);
  }

  // One tiny kraft PAPER peach on the counter so the stall reads as a
  // working stand, not only produce. Kraft body + wood stem. Local
  // offset only — stall world pose stays put. PAPER boxes. Existing
  // hexes. Offset from grape, pear, banana, orange, lime, lemon, plate, napkin, knife, cup.
  if (!g.children.some((c) => c.userData.part === "peach")) {
    const peach = new THREE.Group();
    peach.name = "peach";
    peach.userData.part = "peach";
    peach.userData.mode = "PAPER";
    peach.userData.paper = true;
    peach.position.set(-0.22, 0.9, 1.22);
    const body = paperBox(0.06, 0.055, 0.06, KRAFT, false);
    body.position.y = 0.028;
    body.userData.part = "peach";
    const stem = paperBox(0.015, 0.02, 0.015, WOOD, false);
    stem.position.y = 0.065;
    stem.userData.part = "peach";
    peach.add(body, stem);
    g.add(peach);
  }
  ...
  // One tiny kraft PAPER cherry on the counter so the stall reads as a
  // working stand, not only produce. Kraft body + wood stem. Local
  // offset only — stall world pose stays put. PAPER boxes. Existing
  // hexes. Offset from kiwi, mango, peach, grape, pear, banana, orange,
  // lime, lemon, plate, napkin, knife, cup.
  if (!g.children.some((c) => c.userData.part === "cherry")) {
    const cherry = new THREE.Group();
    cherry.name = "cherry";
    cherry.userData.part = "cherry";
    cherry.userData.mode = "PAPER";
    cherry.userData.paper = true;
    cherry.position.set(0.78, 0.9, 1.22);
    const body = paperBox(0.06, 0.055, 0.06, KRAFT, false);
    body.position.y = 0.028;
    body.userData.part = "cherry";
    const stem = paperBox(0.015, 0.02, 0.015, WOOD, false);
    stem.position.y = 0.065;
  ...
  // One tiny kraft PAPER quince on the counter so the stall reads as a
  // working stand, not only produce. Kraft body + wood stem. Local
  // offset only — stall world pose stays put. PAPER boxes. Existing
  // hexes. Offset from nectarine, cherry, kiwi, mango, peach, grape,
  // pear, banana, orange, lime, lemon, plate, napkin, knife, cup.
  if (!g.children.some((c) => c.userData.part === "quince")) {
    const quince = new THREE.Group();
    quince.name = "quince";
    quince.userData.part = "quince";
    quince.userData.mode = "PAPER";
    quince.userData.paper = true;
    quince.position.set(1.1, 0.9, 1.22);
    const body = paperBox(0.06, 0.055, 0.06, KRAFT, false);
    body.position.y = 0.028;
    body.userData.part = "quince";
    const stem = paperBox(0.015, 0.02, 0.015, WOOD, false);
    stem.position.y = 0.065;
    stem.userData.part = "quince";
    quince.add(body, stem);
    g.add(quince);
  }

  // One tiny kraft PAPER papaya on the counter so the stall reads as a
  // working stand, not only produce. Kraft body + wood stem. Local
  // offset only — stall world pose stays put. PAPER boxes. Existing
  // hexes. Offset from quince, nectarine, cherry, kiwi, mango, peach,
  // grape, pear, banana, orange, lime, lemon, plate, napkin, knife, cup.
  if (!g.children.some((c) => c.userData.part === "papaya")) {
    const papaya = new THREE.Group();
    papaya.name = "papaya";
    papaya.userData.part = "papaya";
    papaya.userData.mode = "PAPER";
    papaya.userData.paper = true;
    papaya.position.set(-1.35, 0.9, 1.22);
    const body = paperBox(0.06, 0.055, 0.06, KRAFT, false);
    body.position.y = 0.028;
    body.userData.part = "papaya";
    const stem = paperBox(0.015, 0.02, 0.015, WOOD, false);
    stem.position.y = 0.065;
    stem.userData.part = "papaya";
    papaya.add(body, stem);
    g.add(papaya);
  }

  // One tiny kraft PAPER guava on the counter so the stall reads as a
  // working stand, not only produce. Kraft body + wood stem. Local
  // offset only — stall world pose stays put. PAPER boxes. Existing
  // hexes. Offset from papaya, quince, nectarine, cherry, kiwi, mango,
  // peach, grape, pear, banana, orange, lime, lemon, plate, napkin,
  // knife, cup.
  if (!g.children.some((c) => c.userData.part === "guava")) {
    const guava = new THREE.Group();
    guava.name = "guava";
    guava.userData.part = "guava";
    guava.userData.mode = "PAPER";
    guava.userData.paper = true;
    guava.position.set(-0.78, 0.9, 1.22);
    const body = paperBox(0.06, 0.055, 0.06, KRAFT, false);
    body.position.y = 0.028;
    body.userData.part = "guava";
    const stem = paperBox(0.015, 0.02, 0.015, WOOD, false);
    stem.position.y = 0.065;
    stem.userData.part = "guava";
    guava.add(body, stem);
    g.add(guava);
  }

  // One tiny kraft PAPER currant on the counter so the stall reads as a
  // working stand, not only produce. Kraft body + wood stem. Local
  // offset only — stall world pose stays put. PAPER boxes. Existing
  // hexes. Offset from guava, papaya, quince, nectarine, cherry, kiwi,
  // mango, peach, grape, pear, banana, orange, lime, lemon, plate,
  // napkin, knife, cup.
  if (!g.children.some((c) => c.userData.part === "currant")) {
    const currant = new THREE.Group();
    currant.name = "currant";
    currant.userData.part = "currant";
    currant.userData.mode = "PAPER";
    currant.userData.paper = true;
    currant.position.set(1.48, 0.9, 1.22);
    const body = paperBox(0.06, 0.055, 0.06, KRAFT, false);
    body.position.y = 0.028;
    body.userData.part = "currant";
    const stem = paperBox(0.015, 0.02, 0.015, WOOD, false);
    stem.position.y = 0.065;
    stem.userData.part = "currant";
    currant.add(body, stem);
    g.add(currant);
  }
