 ...
 * House door only — shop keeps its latch; shed stays bare.
 * Does not replace chimney, mailbox, shutter, knocker, latch, sign, stoop, or doormat.
 */
function addHinge(g, x, y, z) {
  const hinge = tagPaper(part(0.08, 0.16, 0.04, KRAFT, false), "hinge");
  hinge.name = "hinge";
...
/**
 * Tiny kraft PAPER sill under the House door so the leaf reads as
 * a threshold, not a blank plank. Original KRAFT — not a new hex.
 * House door only — shop keeps its latch; shed stays bare.
 * Offset from knob, knocker, hinge, lintel, doormat, and stoop.
 */
function addSill(g, x, y, z) {
  const sill = tagPaper(part(0.72, 0.05, 0.08, KRAFT, false), "sill");
  sill.name = "sill";
...
/**
 * Tiny kraft PAPER jamb beside the House door so the leaf reads as
 * framed, not a blank plank. Original KRAFT — not a new hex.
 * House door only — shop keeps its latch; shed stays bare.
 * Offset from stoop, knocker, hinge, lintel, knob, sill, sconce,
 * transom, doormat, and mailbox.
 */
function addJamb(g, x, y, z) {
  const jamb = tagPaper(part(0.06, 0.36, 0.04, KRAFT, false), "jamb");
  jamb.name = "jamb";
  jamb.position.set(x, y, z);
  g.add(jamb);
}
...
function addKnocker(g, x, y, z) {
  ...
  knocker.userData.part = "knocker";
  knocker.userData.mode = "PAPER";

  const plate = tagPaper(part(0.22, 0.28, 0.04, KRAFT, false), "knocker");
  plate.position.set(x, y, z);
  const boss = tagPaper(part(0.08, 0.08, 0.06, WOOD, false), "knocker");
  boss.position.set(x, y + 0.07, z + 0.04);
  const ring = tagPaper(part(0.14, 0.16, 0.05, WOOD, false), "knocker");
...
function cottage(kind) {
  ...
  if (!shop && !shed) {
    ...
    addJamb(g, -W * 0.12 + 0.52, 1.42, D / 2 + 0.18);
  }
  if (shop) {
