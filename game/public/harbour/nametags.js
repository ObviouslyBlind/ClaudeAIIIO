...
/** Tiny kraft PAPER snap on the card. 3D box, existing walker hex. */
...
/**
 * Tiny kraft PAPER washer on the card face. Kraft 0xc4b496 box —
 * not grey metal. Offset from the loop (0.18, 1.10, 0.05), clasp,
 * bead, rivet, stud, tab, pin, string, and clip. Sprite-local so it
 * stays a washer on the kraft card.
 */
export function makeNametagWasher() {
  const washer = paperBox(0.04, 0.04, 0.02, 0xc4b496);
  washer.name = "paper-nametag-washer";
  washer.userData.part = "washer";
  washer.userData.mode = "PAPER";
  washer.castShadow = false;
  washer.receiveShadow = false;
  washer.frustumCulled = false;
  washer.renderOrder = 4;
  washer.material.depthTest = false;
  washer.position.set(-0.22, 1.12, 0.05);
  return washer;
}
...
/**
 * Tiny kraft PAPER snap on the card face. SKIN kraft box —
 * not grey metal. Offset from the eyelet (0.50, 1.22, 0.05), washer,
 * loop, clasp, bead, rivet, stud, tab, pin, string, and clip.
 * Sprite-local so it stays a snap on the kraft card.
 */
export function makeNametagSnap() {
  const snap = paperBox(0.036, 0.036, 0.02, SKIN);
  snap.name = "paper-nametag-snap";
  snap.userData.part = "snap";
  snap.userData.mode = "PAPER";
  snap.castShadow = false;
  snap.receiveShadow = false;
  snap.frustumCulled = false;
  snap.renderOrder = 4;
  snap.material.depthTest = false;
  snap.position.set(-0.50, 1.30, 0.05);
  return snap;
}

/**
 * Tiny kraft PAPER prong on the card face. PANTS brown box —
 * not grey metal. Offset from the snap (-0.50, 1.30, 0.05), eyelet,
 * washer, loop, clasp, bead, rivet, stud, tab, pin, string, and clip.
 * Sprite-local so it stays a prong on the kraft card.
 */
export function makeNametagProng() {
  const prong = paperBox(0.03, 0.042, 0.018, PANTS);
  prong.name = "paper-nametag-prong";
  prong.userData.part = "prong";
  prong.userData.mode = "PAPER";
  prong.castShadow = false;
  prong.receiveShadow = false;
  prong.frustumCulled = false;
  prong.renderOrder = 4;
  prong.material.depthTest = false;
  prong.position.set(0.50, 0.48, 0.05);
  return prong;
}
