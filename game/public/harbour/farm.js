 ...
 * kraft PAPER cask leaning by the workbench, a tiny
 * kraft PAPER winnow leaning by the workbench, and
...
/**
 * Tiny kraft PAPER harrow: wood beam, kraft teeth.
 * Hexes already in this file (WOOD, KRAFT). PAPER boxes only — no grey.
 * Leans by the farm workbench; offset from fork, flail, yoke, shears,
 * trowel, spade, ladle, scythe, sickle, hoe, rake, broom, seed, lid,
 * mug, egg, pail, lantern, scoop, and the hanging farm-tools. Not the
 * hanging farm-tool shovel. Not the trough pitchfork (farm-fork). Not
 * the tiny kraft PAPER fork or flail.
 */
function farmHarrow() {
  const g = new THREE.Group();
  g.name = "farm-harrow";
  g.userData.kind = "farm-harrow";
  g.userData.part = "harrow";
  g.userData.mode = "PAPER";
  const y0 = 0.16;
  const beam = paperBox(0.10, 0.016, 0.016, WOOD, "farm-harrow");
  beam.userData.part = "harrow";
  beam.position.y = y0 + 0.08;
  g.add(beam);
  const cross = paperBox(0.016, 0.016, 0.055, WOOD, "farm-harrow");
  cross.userData.part = "harrow";
  cross.position.y = y0 + 0.05;
  g.add(cross);
  const teeth = paperBox(0.08, 0.035, 0.05, KRAFT, "farm-harrow");
  teeth.userData.part = "harrow";
  teeth.position.y = y0 + 0.028;
  g.add(teeth);
  return g;
}
...
/**
 * Tiny kraft PAPER hod: kraft basin, wood back, wood handle.
 * Hexes already in this file (WOOD, KRAFT). PAPER boxes only — no grey.
 * Leans by the farm workbench; offset from mattock, harrow, fork, flail,
 * yoke, shears, trowel, spade, ladle, scythe, sickle, hoe, rake, broom,
 * seed, lid, mug, egg, pail, lantern, scoop, and the hanging farm-tools.
 * Not the hanging farm-tool shovel. Not the trough pitchfork (farm-fork).
 * Not the tiny kraft PAPER mattock, harrow, or fork.
 */
function farmHod() {
  const g = new THREE.Group();
  g.name = "farm-hod";
  g.userData.kind = "farm-hod";
  g.userData.part = "hod";
  g.userData.mode = "PAPER";
  const y0 = 0.16;
  const basin = paperBox(0.08, 0.04, 0.055, KRAFT, "farm-hod");
  basin.userData.part = "hod";
  basin.position.y = y0 + 0.03;
  g.add(basin);
  const back = paperBox(0.08, 0.055, 0.016, WOOD, "farm-hod");
  back.userData.part = "hod";
  back.position.set(0, y0 + 0.05, -0.02);
  g.add(back);
  const handle = paperBox(0.016, 0.07, 0.016, WOOD, "farm-hod");
  handle.userData.part = "hod";
  handle.position.set(0, y0 + 0.09, -0.028);
  g.add(handle);
  return g;
}
...
/**
 * Tiny kraft PAPER winnow: kraft tray, wood rim, wood grip.
 * Hexes already in this file (WOOD, KRAFT). PAPER boxes only — no grey.
 * Leans by the farm workbench; offset from cask, hod, mattock, harrow,
 * fork, flail, yoke, shears, trowel, spade, ladle, scythe, sickle, hoe,
 * rake, broom, seed, lid, mug, egg, pail, lantern, scoop, and the
 * hanging farm-tools. Not the hanging farm-tool shovel. Not the trough
 * pitchfork (farm-fork). Not the tiny kraft PAPER cask, hod, mattock,
 * harrow, or fork.
 */
function farmWinnow() {
  const g = new THREE.Group();
  g.name = "farm-winnow";
  g.userData.kind = "farm-winnow";
  g.userData.part = "winnow";
  g.userData.mode = "PAPER";
  const y0 = 0.16;
  const tray = paperBox(0.09, 0.018, 0.065, KRAFT, "farm-winnow");
  tray.userData.part = "winnow";
  tray.position.y = y0 + 0.02;
  g.add(tray);
  const rim = paperBox(0.098, 0.014, 0.072, WOOD, "farm-winnow");
  rim.userData.part = "winnow";
  rim.position.y = y0 + 0.032;
  g.add(rim);
  const grip = paperBox(0.018, 0.05, 0.016, WOOD, "farm-winnow");
  grip.userData.part = "winnow";
  grip.position.set(0, y0 + 0.055, -0.028);
  g.add(grip);
  return g;
}
...
function makeFarmDress() {
  ...
  g.add(paperFork);
  const paperHarrow = farmHarrow();
  // Floor beside the 1.55 × 0.58 bench — leans by the mid-front
  // edge, not the fork (far-left front at -3.96, 0, 0.22), flail
  // (far-left back at -3.95, 0, -0.32), yoke (mid-left at -3.90, 0,
  // 0.0), shears, trowel, spade, ladle, scythe, sickle, hoe, rake,
  // broom, or the top (seed, mug, egg, pail, lantern, scoop). Not
  // hanging farm-tools. Not the trough pitchfork.
  ...
  g.add(paperMattock);
  const paperHod = farmHod();
  // Floor beside the 1.55 × 0.58 bench — leans by the mid-front edge,
  // forward of the mattock (mid-front-left at -3.55, 0, 0.45) and
  // harrow (mid-front at -3.05, 0, 0.48), not the fork (far-left
  // front at -3.96, 0, 0.22), flail, yoke, shears, trowel, spade,
  // ladle, scythe, sickle, hoe, rake, broom, or the top (seed, mug,
  // egg, pail, lantern, scoop). Not hanging farm-tools. Not the
  // trough pitchfork.
  ...
  g.add(paperCask);
  const paperWinnow = farmWinnow();
  // Floor beside the 1.55 × 0.58 bench — leans by the mid-back-left
  // edge, behind the rake (back-left at -3.72, 0, -0.48), not the
  // cask (mid-back-right at -3.00, 0, -0.72), hod (mid-front at
  // -3.36, 0, 0.58), mattock, harrow, fork, flail, yoke, shears,
  // trowel, spade, ladle, scythe, sickle, hoe, broom, or the top
  // (seed, mug, egg, pail, lantern, scoop). Not hanging farm-tools.
  // Not the trough pitchfork.
  paperWinnow.position.set(-3.52, 0, -0.70);
  paperWinnow.rotation.z = 0.22;
  g.add(paperWinnow);
 ...
 * a tiny kraft winnow leaning by the workbench,
