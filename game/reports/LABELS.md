...
| --- | --- | --- | --- |
| World | `world` | the islands | walk. buildings / stands / port. **not** land select |
...
Van follows paved taxi graph → stops on the carriageway (`drop.curbX/Z`) → crate on the verge (`drop.x/z`, `SHOULDER_M`) → van drives away (`drop.awayX/Z`).
