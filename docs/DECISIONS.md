...
## D047 — Taxi fare hint only while the map is open (2026-08-17)

**Decision:** `isTaxiHintActive` is true only when `#taxi-map` is shown. The always-enabled Taxi button no longer paints `$5 · dirt forbidden` on spawn.
**Reason:** HUD critics kept a permanent taxi fare line under Ferry $15, adding another PAPER clone on a sheet that was already too long. PAPER / SIMULATED.

## D046 — Persist line keeps restored after the dump poll (2026-08-17)

**Decision:** After Restore, `formatPersistLine` stays `PAPER · SIMULATED · restored · tick · $`. The 1 s `/api/persist` poll must not clear that phase.
**Reason:** `/g/persist109` FAIL PERSIST: Restore posted and the status note was correct (`Not Postgres. Does not restart play.`), but the persist strip snapped back to the dump line before the critic could read `restored`. PAPER / SIMULATED.
...
**Reason:** Operator screenshot showed a stack of identical `PAPER · SIMULATED` lines (goods + persist + badge). Mixed evidence is not a trade; mixed HUD labels are not a sheet. PAPER / SIMULATED.
